...
void Syntax::Init(AbstractSystem::System s, QSharedPointer<CIniFile> m_ini,
                  QSharedPointer<CIniFile> m_proj) {
  m_currentSystem =
      QSharedPointer<AbstractSystem>(FactorySystem::Create(s, m_ini, m_proj));
...
void Syntax::Reload() {
  SetupReservedWords(reservedWords, "r", false);
  SetupReservedWords(reservedWordsFjong, "rf", true);
  SetupBuiltinFunctions(builtInFunctions, m_currentSystem->m_system, "m",
                        false);
  SetupBuiltinFunctions(builtinFunctionsFjong, m_currentSystem->m_system, "f",
                        true);
  SetupKeys();
  SetupIllegalVariables();
}

void Syntax::SetupReservedWords(QVector<Token> &list, QString id,
                                bool ignoreSystem) {
  list.clear();
  if (m_currentSystem == nullptr)
    return;
  QString currentSystem =
      AbstractSystem::StringFromSystem(m_currentSystem->m_system).toLower();
  for (QString s : m_syntaxData.split('\n')) {
    ...
    if (ignoreSystem ||
        Syntax::s.m_currentSystem->systemIsOfType(system.split(","))) {
      if (data.count() > 3) {
        if (data[3].trimmed() == "f")
...
void Syntax::SetupBuiltinFunctions(QHash<QString, BuiltInFunction> &lst,
...
void Syntax::SetupKeys() {
  int row[8] = {0b11111110, 0b11111101, 0b11111011, 0b11110111,
                0b11101111, 0b11011111, 0b10111111, 0b01111111};
  int column[8] = {0b00000001, 0b00000010, 0b00000100, 0b00001000,
  ...
  m_c64keys[0xEE] =
      C64Key("COMMODORE", "KEY_COMMODORE", 0xEE, row[7], column[5]);
...
bool Syntax::isAlnum(QString s) { return alnum.contains(s.toLower()); }
...
bool Syntax::isString(QString s) { return alnumString.contains(s.toLower()); }

bool Syntax::isAlpha(QString s) { return alpha.contains(s.toLower()); }
