...
//    if (Command("return"))
  //      as->Asm("rts");
    ...
    if (Command("abs")) {
        ...
        QString e = m_codeGen->getEndType(m_node->m_params[0]);
        if (var!=reg)
...
//    LoadVariable(as, "lea", m_node->m_params[0], a0);
 ...
 //   m_codeGen->LoadAddress()
    ...
    if (!(m_node->m_params[1]->isPureNumeric() && m_node->m_params[1]->getValueAsInt(as)==0))
       Asm(as,"add"+m_codeGen->getEndType(m_node->m_params[1]),as->m_varStack.pop(), a0);
...
//    qDebug()  << "methonds68000 a1" << m_node->m_params[2]->getValue(as);
  ...
  //  qDebug()  << "a2";
     m_node->m_params[3]->Accept(m_codeGen);
    if (ok)
        Asm(as,"add"+m_codeGen->getEndType(m_node->m_params[3]),as->m_varStack.pop(), a1);
...
//    m_codeGen->LoadVariable(m_node->m_params[3]);
    ...
    as->Asm("move"+m_codeGen->getEndType(m_node->m_params[3])+" "+ as->m_varStack.pop()+",d7");
