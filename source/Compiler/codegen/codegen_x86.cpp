...
void CodeGenX86::StoreVariable(QSharedPointer<NodeVar> n)
{
    if (n->hasArrayIndex()) {
        if (n->getArrayDataSize(as)==4) {
            if (!n->m_expr->isPure())
                as->Asm("push di");
            n->m_expr->setForceType(TokenType::INTEGER);
...
QString CodeGenX86::getEndType( QSharedPointer<Node> v)
{
    return "";
}
    ...
    if (var->hasArrayIndex()) {
        ...
        if (var->m_expr->isPure()) {
            ...
            if (var->m_expr->isPureNumeric()) {
                ...
                as->Asm("mov di,"+getX86Value(var->m_expr));
                if (var->isWord(as))
                    as->Asm("shl di,1");
            }
        }
...
QString CodeGenX86::getEndType( QSharedPointer<Node> v1, QSharedPointer<Node> v2)
{
    return "";
}
