...
void AbstractCodeGen::dispatch(QSharedPointer<NodeCast> node) {
  ...
  as->Comment("WriteType : " + TokenType::getType(node->getStoreType()) + " " +
      ...
      if (qSharedPointerDynamicCast<NodeVarDecl>(n) == nullptr) {
        if (!blockProcedure) // Print label at end of vardecl
        {
          if (n->m_op.m_lineNumber != 0) {
            ...
            blockProcedure = true;
            //   qDebug() << "pop" << n->m_op.m_lineNumber << " " <<
            //   TokenType::getType(n->getType(as));
          }
        }
      }
      n->Accept(this);
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeForLoop> node) {
  ...
  QSharedPointer<NodeAssign> nVar =
      qSharedPointerDynamicCast<NodeAssign>(node->m_left);
  ...
  if (node->m_unroll) {
    ...
    if (!node->m_right->isPureNumeric())
      ErrorHandler::e.Error(
          "For unrolled loop, right value must be a constant value",
          node->m_op.m_lineNumber);
    if (!node->m_left->m_right->isPureNumeric())
      ErrorHandler::e.Error(
          "For unrolled loop, left value must be a constant value",
          node->m_op.m_lineNumber);
  ...
  if (Syntax::s.m_currentSystem->m_processor == AbstractSystem::MOS6502 &&
      nVar->m_left->isWord(as)) {
    ErrorHandler::e.Warning(
        "Using integer '" + nVar->m_left->getValue(as) +
            "' as a for loop index can result in unpredictable behavior on the "
            "6502. Please keep to using byte indicies, and use pointers to "
            "cover data > 255 bytes. See the TRSE tutorials for examples.",
        node->m_op.m_lineNumber);
  }
 ...
 */
  ...
  CompareAndJumpIfNotEqualAndIncrementCounter(node->m_left, node->m_right,
                                              node->m_step, lblFor, offpage,
                                              node->m_inclusive);
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeBinaryClause> node) {
  ...
  as->Label(lblstartTrueBlock +
            ": ;Main true block ;keep "); // This means skip inside
...
QString AbstractCodeGen::getBank(QSharedPointer<NodeVarType> t) {
  QString bnk =
      t->m_flags[t->m_flags.indexOf("bank") + 1]; // Banks always placed +1
...
void AbstractCodeGen::ValidateAssignStatement(QSharedPointer<NodeAssign> node) {
  ...
  QSharedPointer<NodeNumber> num =
      qSharedPointerDynamicCast<NodeNumber>(node->m_left);
  ...
  if (node->m_left->getType(as) == TokenType::POINTER &&
      node->m_right->isRecord(as) && !node->m_right->isClass(as)) {
    if (!node->m_right->isRecordData(as)) {
      ErrorHandler::e.Error("Cannot assign a pointer data to a record.",
                            node->m_op.m_lineNumber);
  ...
  if (node->m_right == nullptr)
    ErrorHandler::e.Error("Right hand of assign statement must be expression.",
                          node->m_op.m_lineNumber);
...
void AbstractCodeGen::GenericAssign(QSharedPointer<NodeAssign> node) {
  ...
  //  as->Comment("RHS is byte: "+Util::numToHex(node->m_right->isByte(as)) + "
  //  "+node->m_right->getTypeText(as) + " "
  //  +TokenType::getType(node->m_right->getType(as)));
  // as->Comment("LHS is byte: "+Util::numToHex(node->m_left->isByte(as)) + "
  // "+node->m_left->getTypeText(as) + " "
  // +TokenType::getType(node->m_left->getType(as)));
...
void AbstractCodeGen::IncreaseCounter(QSharedPointer<Node> step,
                                      QSharedPointer<Node> var) {

  // no STEP included in FOR TO DO, we assume STEP 1

  if (step == nullptr)
    step = NodeFactory::CreateNumber(var->m_op, 1);
  auto bop = NodeFactory::CreateBinop(var->m_op, TokenType::PLUS, var, step);
  auto assign = NodeFactory::CreateAssign(var->m_op, var, bop);
  assign->Accept(this);
}

void AbstractCodeGen::Compare(QSharedPointer<Node> nodeA,
                              QSharedPointer<Node> nodeB,
                              QSharedPointer<Node> step, bool isLarge,
                              QString loopDone, QString loopNotDone,
                              bool inclusive) {

  Token t = nodeA->m_op;

  auto nasm = NodeFactory::CreateAsm(t, "\t" + getJmp(isLarge) + " " +
                                            loopNotDone + "\n");
  auto block = NodeFactory::CreateBlockFromStatements(
      t, QVector<QSharedPointer<Node>>() << nasm);

  if (inclusive)
    nodeB = NodeFactory::CreateBinop(t, TokenType::PLUS, nodeB,
                                     NodeFactory::CreateNumber(t, 1));

  auto cond = NodeFactory::CreateSingleConditional(
      t, TokenType::EQUALS, isLarge, nodeA->m_left, nodeB, block);

  cond->Accept(this);
}

void AbstractCodeGen::SmallLoop(QSharedPointer<NodeForLoop> node,
                                QSharedPointer<NodeVar> var, bool inclusive) {
...
void AbstractCodeGen::LargeLoop(QSharedPointer<NodeForLoop> node,
                                QSharedPointer<NodeVar> var, bool inclusive) {
...
QSharedPointer<NodeVar> AbstractCodeGen::VarOrNum(QSharedPointer<Node> node) {
  ...
  if (num != nullptr) {
    ...
    v = QSharedPointer<NodeVar>(
        new NodeVar(num->m_op)); // Create a variable copy
...
void AbstractCodeGen::AssignPureRecords(QSharedPointer<NodeAssign> node) {
  ...
  if (!node->m_right->isRecord(as))
    ErrorHandler::e.Error(
        "Right-hand side of assignment must also be record of type '" +
            v->getTypeText(as) + "'",
        v->m_op.m_lineNumber);
  if (v->getTypeText(as) != node->m_right->getTypeText(as))
    ErrorHandler::e.Error(
        "Right-hand side of assignment must also be of type '" +
            v->getTypeText(as) + "'",
        v->m_op.m_lineNumber);
  if (node->m_right->isRecordData(as))
    ErrorHandler::e.Error(
 ...
 *
 * Handles everything of tye type A := B; in TRSE. Which is.. 96% of the
 * language.
 *
 */
...
*/
  ...
  //    qDebug() <<v->value<<TokenType::getType(v->m_classvariableType)
  //    <<TokenType::getType(node->m_right->getClassvariableType());
...
*/
  ...
  if (qSharedPointerDynamicCast<NodeString>(node->m_right)) {
    ...
    //        qDebug() << v->getValue(as) <<v->getTypeText(as) <<
    //        TokenType::getType(v->getArrayType(as)) <<
    //        TokenType::getType(Syntax::s.m_currentSystem->getSystemPointerArrayType());
    if (v->hasArrayIndex() &&
        v->getArrayType(as) !=
            Syntax::s.m_currentSystem->getSystemPointerArrayType())
      ErrorHandler::e.Error("Can only assign strings to arrays of pointers "
                            "(such as string lists)",
                            v->m_op.m_lineNumber);
    if (!v->hasArrayIndex() &&
        (!(v->isPointer(as) || v->getType(as) == TokenType::STRING)))
      ErrorHandler::e.Error(
  ...
  if (!ignoreLookup)
    QSharedPointer<Symbol> s = as->m_symTab->Lookup(
        getValue(v), node->m_op.m_lineNumber, v->isAddress());
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeConditional> node) {
  ...
  QSharedPointer<NodeBinaryClause> bn =
      qSharedPointerDynamicCast<NodeBinaryClause>(node->m_binaryClause);
...
#include <iostream>
void AbstractCodeGen::HandleCompoundBinaryClause(QSharedPointer<Node> node,
                                                 QString lblFailed,
                                                 QString lblSuccess,
                                                 bool offpage) {
  ...
  if (!node->isCompoundClause()) { // IS LEAF
    ...
    if (node->m_right->m_isBoolean) {
      //            qDebug() << "Right is boolean!" <<
      //            node->m_right->getValue(as);
  ...
  if (node->m_op.m_type == TokenType::AND) {
    ...
    //        HandleCompoundBinaryClause(node->m_left,  lblFailed, lblSuccess,
    //        offpage);
    HandleCompoundBinaryClause(node->m_left, lblFailed, lblLocalSuccess,
                               offpage);
  ...
  if (node->m_op.m_type == TokenType::OR) {
    ...
    HandleCompoundBinaryClause(node->m_left, lblLocalFailed, lblSuccess,
                               offpage);
...
/*
 *
 *  Determines if a node block is >~127 bytes or not (for relative branching)
 * (offpage/onpage)
 *
 * */

bool AbstractCodeGen::isOffPage(QSharedPointer<Node> node,
                                QSharedPointer<Node> b1,
                                QSharedPointer<Node> b2) {
  //  bool onPage = true;
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeProcedureDecl> node) {
  ...
  //    qDebug() << " AbstractCodeGen HERE "<< node->m_procName <<
  //    node->m_isInline;
  node->DispatchConstructor(as, this);
  ...
  if (node->m_flags.contains("bank")) {
    QString bnk = node->m_flags[node->m_flags.indexOf("bank") +
                                1]; // Banks always placed +1
    isInbank = true;
    if (!as->m_banks.contains(bnk)) {
      as->m_banks[bnk] = QSharedPointer<Appendix>(new Appendix());
      //            as->m_banks[bnk]->m_pos = "$4000";
    ...
    as->Comment("NodeProcedureDecl " +
                QString::number(node->getBlockInfo().m_blockID));
    int ret = node->MaintainBlocks(as);
    if (ret == 3)
    ...
    if (as->m_currentBlock != nullptr) {
      if (node->m_curMemoryBlock == nullptr) {
        bool ok;
        //            qDebug() << "Creating new block procedure for " <<
        //            m_procName;
        QString p = as->m_currentBlock->m_pos;
        int pos = p.remove("$").toInt(&ok, 16);
        //                as->StartMemoryBlock()
        node->m_curMemoryBlock = QSharedPointer<MemoryBlock>(new MemoryBlock(
            pos, pos, MemoryBlock::CODE, node->getBlockInfo().m_blockName));
  ...
  if (node->m_block == nullptr) { // Is builtin procedure
    node->m_block = QSharedPointer<NodeBuiltinMethod>(new NodeBuiltinMethod(
        node->m_procName, QVector<QSharedPointer<Node>>(), nullptr));
    //        Node::s_uniqueSymbols[node->m_block] = node->m_block; // Mark for
    //        deletion
  }
  ...
  if (Syntax::s.builtInFunctions.contains(node->m_procName)) {
    ...
    isInitFunction =
        Syntax::s.builtInFunctions[node->m_procName].m_initFunction;
    //        as->EndMemoryBlock();
  }
  ...
  QString type =
      (isBuiltinFunction) ? "Built-in function" : "User-defined procedure";
  ...
  */
  ...
  if (node->m_block != nullptr) {
    QSharedPointer<NodeBlock> b =
        qSharedPointerDynamicCast<NodeBlock>(node->m_block);
    if (b != nullptr) {
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeProcedure> node) {
  ...
  if (node->m_parameters.count() != node->m_procedure->m_paramDecl.count())
    ErrorHandler::e.Error(
        "Procedure '" + node->m_procedure->m_procName + "' requires " +
            QString::number(node->m_procedure->m_paramDecl.count()) +
            " parameters, not " + QString::number(node->m_parameters.count()) +
            ".",
        node->m_op.m_lineNumber);

  // Now, check for pureness
  for (int i = 0; i < node->m_parameters.count(); i++) {
    QSharedPointer<NodeVarDecl> vd = qSharedPointerDynamicCast<NodeVarDecl>(
        node->m_procedure->m_paramDecl[i]);
    QSharedPointer<NodeVarType> vt =
        qSharedPointerDynamicCast<NodeVarType>(vd->m_typeNode);
    //        qDebug() << vt->m_flags.contains("pure_number") <<
    //        node->m_procedure->m_procName;
    if (vt->m_flags.contains("pure"))
      if (!node->m_parameters[i]->isPure())
        ErrorHandler::e.Error("Procedure '" + node->m_procedure->m_procName +
                                  "' requires parameter " + QString::number(i) +
                                  " to be a pure variable or number",
                              node->m_op.m_lineNumber);
    if (vt->m_flags.contains("pure_number"))
      if (!node->m_parameters[i]->isPureNumeric())
        ErrorHandler::e.Error("Procedure '" + node->m_procedure->m_procName +
                                  "' requires parameter " + QString::number(i) +
                                  " to be a pure number / constant",
                              node->m_op.m_lineNumber);
    if (vt->m_flags.contains("pure_variable"))
      if (!node->m_parameters[i]->isPureVariable())
        ErrorHandler::e.Error("Procedure '" + node->m_procedure->m_procName +
                                  "' requires parameter " + QString::number(i) +
                                  " to be a pure variable",
                              node->m_op.m_lineNumber);
    //        qDebug() <<"typenode flags"
    //        <<node->m_procedure->m_procName<<vt->m_flags <<
    //        vd->m_varNode->getValue(as);
  }
  ...
  for (int i = 0; i < node->m_parameters.count(); i++) {
    ...
    QSharedPointer<NodeAssign> na = QSharedPointer<NodeAssign>(new NodeAssign(
        vd->m_varNode, node->m_parameters[i]->m_op, node->m_parameters[i]));
...
void AbstractCodeGen::InlineProcedure(QSharedPointer<NodeProcedure> p) {
  ...
  // Just call it once
  p->m_procedure->m_block->ReplaceInlineAssemblerVariables(as, "Nothing",
                                                           "Nothing");
  ...
  qSharedPointerDynamicCast<NodeBlock>(p->m_procedure->m_block)
      ->m_ignoreDeclarations = true;
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeVarDecl> node) {
    ...
    if (node->m_curMemoryBlock != nullptr) {
      if (as->m_currentBlock != nullptr) {
        if (node->m_curMemoryBlock == nullptr) {
          ...
          int pos = p.remove("$").toInt(&ok, 16);
          node->m_curMemoryBlock = QSharedPointer<MemoryBlock>(new MemoryBlock(
              pos, pos, MemoryBlock::ARRAY, node->getBlockInfo().m_blockName));
  ...
  QSharedPointer<NodeVar> v =
      qSharedPointerDynamicCast<NodeVar>(node->m_varNode);
  QSharedPointer<NodeVarType> t =
      qSharedPointerDynamicCast<NodeVarType>(node->m_typeNode);
  ...
  if (t->m_op.m_type == TokenType::ARRAY) {
    ...
    QSharedPointer<Symbol> s =
        as->m_symTab->Lookup(v->value, node->m_op.m_lineNumber);
    s->isUsed = false;
    s->m_arrayTypeText = s->m_type;
    s->m_type = "address";
    s->m_arrayType = t->m_arrayVarType.m_type;
  } else if (t->m_op.m_type == TokenType::STRING) {
    ...
    node->m_dataSize++; // 0 end
                        //      qDebug() << as->m_mainBlock->m_source;
  } else if (t->m_op.m_type == TokenType::CSTRING) {
    as->DeclareCString(v->value, t->m_data, t->m_flags);
    node->m_dataSize = 0;
    for (QString s : t->m_data)
      node->m_dataSize += s.length();
    node->m_dataSize++; // 0 end
  } else if (t->m_op.m_type == TokenType::INCBIN) {
    ...
    as->m_symTab->Lookup(v->getValue(as), node->m_op.m_lineNumber)
        ->m_arrayType = t->m_arrayVarType.m_type;
             ...
             t->m_op.m_type == TokenType::INCNSF) {
    // Ignore
  } else {
    node->m_dataSize = 1;
    if (t->getValue(as).toLower() == "integer")
      node->m_dataSize = 2;
    if (t->getValue(as).toLower() == "long")
      node->m_dataSize = 4;
    ...
    if (t->m_flag == 1)
      t->initVal = Util::numToHex(Util::NumberFromStringHex(t->initVal) +
                                  node->m_dataSize);
  }
...
void AbstractCodeGen::IncBin(QSharedPointer<NodeVarDecl> node) {
  ...
  if (!QFile::exists(filename)) {
    ...
    filename =
        Util::GetSystemPrefix() + Data::data.unitPath + QDir::separator() +
        AbstractSystem::StringFromSystem(Syntax::s.m_currentSystem->m_system) +
        QDir::separator() +
        t->m_filename.replace("\\", "/"); // Test in Units name
    if (!QFile::exists(filename))
      ErrorHandler::e.Error("Could not locate binary file for inclusion :" +
                                t->m_filename,
                            node->m_op.m_lineNumber);
  }
  ...
  bool ok = true;
    // default position is at current mempos
  int pos = -1;
  QString spos = t->m_position;
  // find actual specified position if not zero
  if (t->m_position!="")
     pos = spos.remove("$").toInt(&ok,16);
    // if -1, then 0xFFFF
  if (!ok)
      ErrorHandler::e.Error("Incorrect position of incbin", t->m_currentLineNumber);
  ...
  if (pos==-1 || pos==0xFFFF) {
    ...
    QSharedPointer<Appendix> app =
        QSharedPointer<Appendix>(new Appendix(t->m_position));

    QSharedPointer<Symbol> typeSymbol =
        as->m_symTab->Lookup(v->value, node->m_op.m_lineNumber);
    ...
    if (t->m_position.startsWith("$")) {
      ...
      start = t->m_position.toInt();

    app->Append("end_incbin_" + v->value + ":", 0);

    as->blocks.append(QSharedPointer<MemoryBlock>(
        new MemoryBlock(start, start + size, MemoryBlock::DATA, filename)));
...
void AbstractCodeGen::HandleNodeAssignCopyRecord(
  ...
  for (QSharedPointer<Symbol> s : stab->m_symbols) {
    QSharedPointer<NodeVar> l = QSharedPointer<NodeVar>(
        new NodeVar(Token(TokenType::ID, getValue(node->m_left))));
    l->m_op.m_lineNumber = node->m_op.m_lineNumber;
    l->m_expr = qSharedPointerDynamicCast<NodeVar>(node->m_left)->m_expr;
    QSharedPointer<NodeVar> lp =
        QSharedPointer<NodeVar>(new NodeVar(Token(TokenType::ID, s->m_name)));
    l->m_subNode = lp;

    QSharedPointer<NodeVar> r = QSharedPointer<NodeVar>(
        new NodeVar(Token(TokenType::ID, getValue(node->m_right))));
    QSharedPointer<NodeVar> rp =
        QSharedPointer<NodeVar>(new NodeVar(Token(TokenType::ID, s->m_name)));
    r->m_subNode = rp;
    r->m_op.m_lineNumber = node->m_op.m_lineNumber;
    r->m_expr = qSharedPointerDynamicCast<NodeVar>(node->m_right)->m_expr;

    QSharedPointer<NodeAssign> ns =
        QSharedPointer<NodeAssign>(new NodeAssign(l, node->m_op, r));
    ns->Accept(this);
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeUnaryOp> node) {
  ...
  //    QSharedPointer<NodeNumber> num =
  //    qSharedPointerDynamicCast<NodeNumber>(node->m_right);
...
bool AbstractCodeGen::Evaluate16bitExpr(QSharedPointer<Node> node, QString &lo,
                                        QString &hi) {
  if (node->isPure()) {
...
bool AbstractCodeGen::Evaluate24bitExpr(QSharedPointer<Node> node, QString &lo,
                                        QString &hi, QString &z) {
  if (node->isPure()) {
    lo = getValue8bit(node, false);
    hi = getValue8bit(node, true);
...
void AbstractCodeGen::dispatch(QSharedPointer<NodeCase> node) {
  ...
  if (!expr->isPureVariable()) {
    ...
    //        if
    //        (Syntax::s.m_currentSystem->m_processor!=AbstractSystem::MOS6502)
    //          ErrorHandler::e.Error("Cases can only operate on pure variables
    //          (for now). Not implemented on the z80 / m68k / x86 yet, please
    //          nag the developer!",node->m_op.m_lineNumber);
