...
Compiler::Compiler(QSharedPointer<CIniFile> ini,
                   QSharedPointer<CIniFile> pIni) {
  m_ini = ini;
  m_projectIni = pIni;
...
void Compiler::Parse(QString text, QStringList lst, QString fname) {
  ...
  connect(&m_parser, SIGNAL(emitRequestSystemChange(QString)), this,
          SLOT(AcceptRequestSystemChange(QString)));
  Data::data.genLabel = 0;
  //   SymbolTable::m_forwardedSymbols.clear();

  m_tree = nullptr;
  m_parser.m_preprocessorDefines[m_projectIni->getString("system").toUpper()] =
      "1";
  ...
  try {
    m_tree = m_parser.Parse(
        m_projectIni->getdouble("remove_unused_symbols") == 1.0 &&
...
bool Compiler::Build(QSharedPointer<AbstractSystem> system,
                     QString project_dir) {
  if (m_tree == nullptr) {
    return false;
  }
  ...
  try {
    ...
    m_assembler->m_disableComments =
        m_projectIni->getdouble("disable_compiler_comments") == 1;
  ...
  // COPY SYMBOL TABLE
  m_assembler->m_symTab->m_forwardedVariables =
      m_parser.m_symTab->m_forwardedVariables;

  for (QString s : m_parser.m_symTab->m_forwardedVariables) {
    m_assembler->m_symTab->m_forwardedSymbols[s] =
        m_parser.m_symTab->m_symbols[s];
  }

  //    m_assembler->m_symTab->m_forwardedSymbols =
  //    m_parser.m_symTab->m_forwardedSymbols;
  ...
  m_assembler->m_symTab->m_constants =
      m_parser.m_symTab->m_constants; // Write init assembler code
  m_assembler->m_symTab->m_units = Parser::s_usedTRUNames;

  for (QSharedPointer<SymbolTable> st : m_parser.m_symTab->m_records)
    m_assembler->m_symTab->Define(
        QSharedPointer<Symbol>(new Symbol(st->m_name, "RECORD")));

  connect(m_codeGen.get(), SIGNAL(EmitTick(QString)), this,
          SLOT(AcceptDispatcherTick(QString)));
  SymbolTable::pass = 1;

  emit EmitTick("<br>Building []");
  if (m_tree != nullptr)
    try {
      qSharedPointerDynamicCast<NodeProgram>(m_tree)->m_initJumps =
          m_parser.m_initJumps;
  ...
  disconnect(m_codeGen.get(), SIGNAL(EmitTick(QString)), this,
             SLOT(AcceptDispatcherTick(QString)));
  disconnect(&m_parser, SIGNAL(emitRequestSystemChange(QString)), this,
             SLOT(AcceptRequestSystemChange(QString)));
  ...
  if (m_assembler->m_optimiser != nullptr &&
      m_ini->getdouble("post_optimize") == 1.0) {
    m_assembler->m_source =
        m_assembler->m_optimiser->PostOptimize(m_assembler->m_source);
    m_assembler->m_totalOptimizedLines +=
        m_assembler->m_optimiser->m_linesOptimized;
  }
...
bool Compiler::SetupMemoryAnalyzer(QString filename, Orgasm *orgAsm) {
    ...
    if (s.toLower().startsWith("startblock")) {
      ...
      if (start != end) {
        if (m_assembler != nullptr) {
          ...
          if (ok) {
            m_assembler->blocks.append(QSharedPointer<MemoryBlock>(
                new MemoryBlock(start, end, MemoryBlock::CODE, name)));
          }
        }
      }
    }
  ...
  std::sort(
      m_assembler->blocks.begin(), m_assembler->blocks.end(),
  ...
  for (int i = 0; i < m_assembler->blocks.count(); i++) {
    ...
    for (int j = i + 1; j < m_assembler->blocks.count(); j++) {
      ...
      if (x->m_start < y->m_end && y->m_start < x->m_end) {
        ...
        ErrorHandler::e.Warning("Overlapping memory regions: '" + x->m_name +
                                "' and '" + y->m_name + "' at " +
                                Util::numToHex(y->m_start) + " to " +
                                Util::numToHex(x->m_end) +
                                ". See the memory analyzer for details.");
      }
    }
  }
...
void Compiler::AcceptDispatcherTick(QString val) { emit EmitTick(val); }
