...
//    if (reg[1]=='h' || reg[1]=='l')
    if (reg.count()>=2) {
        if (val.startsWith("$")) {
        line.m_changeRegs[QString(reg[0])] = QString::number(Util::NumberFromStringHex(val)&0xFF);
        line.m_changeRegs[QString(reg[1])] = QString::number((Util::NumberFromStringHex(val)>>8)&0xFF);
        }
        else {
