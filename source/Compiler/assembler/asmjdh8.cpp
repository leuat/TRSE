...
void AsmJDH8::Write(QString str, int level)
{
//    QString s = str.replace("$","0x");
    Assembler::Write(str,level);
}

void AsmJDH8::DeclareArray(QString name, QString type, int count, QStringList data, QString pos)
{
...
void AsmJDH8::DeclareVariable(QString name, QString type, QString initval, QString position)
{
    QString t = "";
...
void AsmJDH8::BinOP(TokenType::Type t,  bool clearFlag)
{
    if (t == TokenType::PLUS) {
        m_term = "add ";
    }
...
int AsmJDH8::CodeSizeEstimator(QStringList &lines) {
    int size = 0;
    //   qDebug() << "EST START";
    for (auto&s : lines) {
...
void AsmJDH8::Label(QString s)
{
    Write(s+":",0);
}

QString AsmJDH8::GetOrg(int pos)
{
    if (Syntax::s.m_currentSystem->m_system == AbstractSystem::GAMEBOY)
...
QString AsmJDH8::String(QStringList lst, bool term)
{
...
void AsmJDH8::StartMemoryBlock(QString pos) {
