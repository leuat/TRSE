...
QSharedPointer<Node>
Parser::ManageClassProcedureCalls(QSharedPointer<Node> left) {
    ...
    ;
    v->value = s->m_name;
    //    qDebug() << "PARSER " <<p->m_procedure->m_procName<< v->value<<s->m_name
    //    << s->m_type << s->m_arrayTypeText <<s->m_pointsTo <<s->m_type.toLower()
    //    <<s->getEndType();
...
void Parser::InitObsolete() {
    ...
    m_obsoleteWarnings.append(QStringList()
                              << "rand"
                              << "Funtion 'Rand()' is scheduled to be "
                                 "deprecated. Please use 'Random()' instead. ");
    m_obsoleteWarnings.append(
        QStringList()
        << "getkey"
        << "Funtion 'GetKey()' is scheduled to be deprecated soon. Please use "
           "the generic system units instead (input/key) etc ");
    //    m_obsoleteWarnings.append(QStringList() << "writeln"<<"Funtion
    //    'Writeln()' is scheduled to be deprecated from 0.09. Please use
    //    'PrintString()' instead. "); m_obsoleteWarnings.append(QStringList() <<
    //    "copycharsetfromrom"<<"Funtion 'CopyCharsetFromROM()' is scheduled to be
    //    deprecated from 0.09. ");
}
...
void Parser::Eat(TokenType::Type t) {
    ...
    //   TokenType::getType(m_currentToken.m_type);
    if (m_currentToken.m_type == t) {
        m_currentToken = m_lexer->GetNextToken();
        int cnt = 0;
        while (m_currentToken.m_type == TokenType::PREPROCESSOR &&
               m_pass > PASS_PREPRE && m_pass != PASS_OTHER && cnt++ < 32) {
            HandlePreprocessorInParsing();
        }

    } else {
        QString warning = "\nDid you forget a semicolon (;) ?";
        ErrorHandler::e.Error("Expected '" + TokenType::getType(t) +
                                  "' but found '" + m_currentToken.m_value + "'" +
                                  warning,
                              m_currentToken.m_lineNumber);
    }
}

void Parser::Eat() { Eat(m_currentToken.m_type); }
...
void Parser::InitBuiltinFunctions() {
    ...
    if (Syntax::s.m_currentSystem->m_system == AbstractSystem::AMIGA) {
        ...
        InitBuiltinFunction(QStringList() << "playp61module" << "initp61module",
    ...
    */
        ...
        InitBuiltinFunction(QStringList() << "decrunch(" << "decrunchfromindex(",
                            "init_decrunch");

        if (Syntax::s.m_currentSystem->m_system != AbstractSystem::NES &&
            Syntax::s.m_currentSystem->m_system != AbstractSystem::ATARI520ST)
        ...
        InitBuiltinFunction(QStringList() << "moveto(" << "printstring(" << "tile(",
                            "initmoveto");
        InitBuiltinFunction(QStringList() << "printstring(" << "printnumber(",
                            "initprintstring");

        InitBuiltinFunction(QStringList() << "drawtextbox(" << "drawcolortextbox(",
                            "initdrawtextbox");
        ...
        InitBuiltinFunction(QStringList() << "vbmtesttilepixel(",
                            "initVbmTestTilePixel");
        InitBuiltinFunction(QStringList() << "vbmtesttilepixel2(",
                            "initVbmTestTilePixel2");

        InitBuiltinFunction(QStringList() << "vbmsetdisplaymode(", "initVbm");
        InitBuiltinFunction(QStringList()
                                << "vbmsetposition1(" << "vbmsetposition2("
                                << "vbmsetposition4(",
                            "initVbmSetPosition");
        ...
        InitBuiltinFunction(QStringList() << "vbmdrawdot(" << "vbmcleardot("
                                          << "vbmdrawdote(" << "vbmtestpixel(",
                            "initVbmDot");
        ...
        InitBuiltinFunction(QStringList() << "vbmdrawblot(" << "vbmclearblot("
                                          << "vbmdrawblote(" << "vbmtestpixel2(",
                            "initVbmBlot");

        InitBuiltinFunction(QStringList() << "vbmscrollleft(", "initVbmScrollLeft");
        InitBuiltinFunction(QStringList() << "vbmscrollright(",
                            "initVbmScrollRight");
        InitBuiltinFunction(QStringList() << "vbmscrollfixtop(",
                            "initVbmScrollFixTop");
        InitBuiltinFunction(QStringList() << "vbmscrollfixbottom(",
                            "initVbmScrollFixBottom");

        InitBuiltinFunction(QStringList() << "vbmdrawtilemap(",
                            "initVbmDrawTileMap");
        InitBuiltinFunction(QStringList() << "vbmdrawtilemapo(",
                            "initVbmDrawTileMapO");
        InitBuiltinFunction(QStringList() << "vbmdrawtilemape(",
                            "initVbmDrawTileMapE");
        InitBuiltinFunction(QStringList() << "vbmcleartilemap(",
                            "initVbmClearTileMap");

        // must now call manually to set up for screen mode
        // InitBuiltinFunction(QStringList()<< "vbmscreenshiftleft(",
        // "initVbmScreenShiftLeft"); InitBuiltinFunction(QStringList()<<
        // "vbmscreenshiftright(", "initVbmScreenShiftRight");

        InitBuiltinFunction(QStringList() << "vbmspritestitch(",
                            "initVbmSpriteStitch");
        InitBuiltinFunction(QStringList() << "vbmspriteshiftr(",
                            "initVbmSpriteShiftR");
        InitBuiltinFunction(QStringList() << "vbmspriteshiftl(",
                            "initVbmSpriteShiftL");
        InitBuiltinFunction(QStringList() << "vbmspriteshiftsr(",
                            "initVbmSpriteShiftSR");
        InitBuiltinFunction(QStringList() << "vbmspriteshiftsl(",
                            "initVbmSpriteShiftSL");

        InitBuiltinFunction(QStringList() << "vbmdrawsprite8(",
                            "initVbmDrawSprite8");
        InitBuiltinFunction(QStringList() << "vbmdrawsprite8e(",
                            "initVbmDrawSprite8E");
        InitBuiltinFunction(QStringList() << "vbmclearsprite8(",
                            "initVbmClearSprite8");

        InitBuiltinFunction(QStringList() << "vbmdrawsprite(", "initVbmDrawSprite");
        InitBuiltinFunction(QStringList() << "vbmdrawspritee(",
                            "initVbmDrawSpriteE");
        InitBuiltinFunction(QStringList() << "vbmclearsprite(",
                            "initVbmClearSprite");

        InitBuiltinFunction(QStringList() << "vbmdrawsprite2(",
                            "initVbmDrawSprite2");
        InitBuiltinFunction(QStringList() << "vbmdrawsprite2e(",
                            "initVbmDrawSprite2E");
        InitBuiltinFunction(QStringList() << "vbmclearsprite2(",
                            "initVbmClearSprite2");

        InitBuiltinFunction(QStringList() << "vbmdrawsprite16(",
                            "initVbmDrawSprite16");
        InitBuiltinFunction(QStringList() << "vbmdrawsprite16e(",
                            "initVbmDrawSprite16E");
        InitBuiltinFunction(QStringList() << "vbmclearsprite16(",
                            "initVbmClearSprite16");

        InitBuiltinFunction(QStringList() << "vbmdrawspriteslice(",
                            "initVbmDrawSpriteSlice");
        InitBuiltinFunction(QStringList() << "vbmdrawspriteslicee(",
                            "initVbmDrawSpriteSliceE");
        InitBuiltinFunction(QStringList() << "vbmclearspriteslice(",
                            "initVbmClearSpriteSlice");
        ...
        InitBuiltinFunction(QStringList() << "vbmdrawsmalltexto(",
                            "initVbmDrawSmallTextO");
        InitBuiltinFunction(QStringList() << "vbmdrawsmalltexte(",
                            "initVbmDrawSmallTextE");
        InitBuiltinFunction(QStringList() << "vbmclearsmalltext(",
                            "initVbmClearSmallText");

        InitBuiltinFunction(QStringList() << "vbmdrawbcd(", "initVbmDrawBCD");
        InitBuiltinFunction(QStringList() << "vbmdrawsmallbcd(",
                            "initVbmDrawSmallBCD");
        InitBuiltinFunction(QStringList() << "vbmdrawsmallbcdo(",
                            "initVbmDrawSmallBCDO");

        InitBuiltinFunction(QStringList() << "vbmcopytobuffer(",
                            "initVbmCopyToBuffer");
        InitBuiltinFunction(QStringList() << "vbmcopyfrombuffer(",
                            "initVbmCopyFromBuffer");
    }
    if (m_preprocessorDefines.contains("BuiltinMethodsLocation")) {
...
void Parser::InitBuiltinFunction(QStringList methodName,
                                 QString builtinFunctionName,
                                 QString initJump) {
    ...
    for (QString s : methodName)
        if (txt.contains(s)) {
            m_procedures[builtinFunctionName] =
                QSharedPointer<NodeProcedureDecl>(new NodeProcedureDecl(
...
void Parser::InitSystemPreprocessors() {
    m_preprocessorDefines[AbstractSystem::StringFromSystem(
        Syntax::s.m_currentSystem->m_system)] = "1";
    m_preprocessorDefines["CPU_" + AbstractSystem::StringFromProcessor(
                              Syntax::s.m_currentSystem->m_processor)] =
        "1";
    if (Syntax::s.m_currentSystem->getCPUFlavor() != "")
        m_preprocessorDefines["CPU_FLAVOR_" +
                              Syntax::s.m_currentSystem->getCPUFlavor()] = "1";
...
void Parser::PreprocessIfDefs(bool ifdef) {
    ...
    if (m_currentToken.m_value == "else") {
        ...
        if (isIf) {
            ...
            if (keyValue == "")
                keyValue = QString::number(m_currentToken.m_intVal);
            //             Eat();
        }
    }
        ...
        if (m_currentToken.m_type == TokenType::PREPROCESSOR) {
            ...
            if (m_currentToken.m_value == "endif") {
                if (ignorePop == 0) {
                    ...
                    m_ignoreAll = false;
                    if (m_lastKey.count() == 0)
                        ErrorHandler::e.Error("Preprocessor '@endif' mismatch error",
                                              m_currentToken.m_lineNumber);
...
void Parser::ApplyTPUAfter(QVector<QSharedPointer<Node>> &declBlock,
    ...
    for (QSharedPointer<Parser> p : m_tpus) {
        ...
        QSharedPointer<NodeProgram> np =
            qSharedPointerDynamicCast<NodeProgram>(p->m_tree);
        ...
        for (QSharedPointer<Node> on : np->m_NodeBlock->m_decl) {
            ...
            if (procDecl != nullptr) {
                if (!m_ignoreBuiltinFunctionTPU.contains(procDecl->m_procName)) {
                    ...
                    //                    qDebug() << "Appending : " <<
                    //                    procDecl->m_procName << procDecl->m_isUsed;
        ...
        declBlock.clear();
        // Make sure that TRU variables are declared FIRST. Ordering is important
        // for the GB
...
int Parser::getParsedNumberOrConstant() {
    ...
    QSharedPointer<Symbol> s =
        m_symTab->LookupConstants(m_currentToken.m_value.toUpper());
    if (s == nullptr)
        ErrorHandler::e.Error("Value required to be a number or a constant.",
                              m_currentToken.m_lineNumber);
...
int Parser::GetParsedInt(TokenType::Type forceType) {
    ...
    while (!done) {
        ...
        if (m_currentToken.m_type == TokenType::COMMA ||
            ...
            m_currentToken.m_type == TokenType::RBRACKET) {
            if (p != 0)
                ErrorHandler::e.Error("Mismatced paranthesis when parsing number",
                                      m_currentToken.m_lineNumber);
            else {
                done = true;
                continue;
        ...
        if (prevNumber == true) {
            ...
            ErrorHandler::e.Error("Error parsing number : " + str,
                                  m_currentToken.m_lineNumber);
        }

        if (m_currentToken.m_value == "") {
            //            qDebug() << "ival:"  <<
            //            QString::number(m_currentToken.m_intVal);
            str += QString::number(m_currentToken.m_intVal);
            Eat();
            prevNumber = true;
        } else {

            //            qDebug() << "Looking for constant " <<
            //            m_currentToken.m_value.toUpper();
            ...
            /*              if (s==nullptr) {
            s =
        m_symTab->Lookup(m_currentToken.m_value,m_currentToken.m_lineNumber);
            isConst = false;
        }
*/
            if (s == nullptr) {
                done = true;
                ErrorHandler::e.Error("Could not find constant/symbol : '" +
                                          m_currentToken.m_value + "'",
                                      m_currentToken.m_lineNumber);
                ...
                //             ErrorHandler::e.Error("Value required to be a number or a
                //             constant.",m_currentToken.m_lineNumber);
            }
        }
    }
    //    if (p!=0)
    ...
    int r = ret.toInt();
    //    qDebug() << "PARSER "<<r;
    if (forceType == TokenType::ADDRESS &&
        Syntax::s.m_currentSystem->m_system == AbstractSystem::OK64)
        return r;
    if (forceType == TokenType::ADDRESS &&
        Syntax::s.m_currentSystem->addressBusBits() == 32) {
        /*        if (r<0) {
                r = r + 0x100000000;
                qDebug() << "HEERE";
            }*/
    ...
    if (forceType == TokenType::ADDRESS &&
        Syntax::s.m_currentSystem->addressBusBits() > 16)
        return r;
...
int Parser::findPage() {
    ...
    if (m_currentToken.m_type == TokenType::OFFPAGE) {
        forcePage = 1;
        Eat();
    } else if (m_currentToken.m_type == TokenType::ONPAGE) {
        forcePage = 2;
        Eat();
    }
...
void Parser::VerifyTypeSpec(Token &t) {
    ...
    if (Syntax::s.keywords.contains(m_symTab->strip(t.m_value).toLower())) {
        ErrorHandler::e.Error("'" + m_symTab->strip(t.m_value).toLower() +
                                  "' is a reserved keyword, not a type.",
                              m_currentToken.m_lineNumber);
    }
    try {
...
*/
        ...
        if (!ok)
            if (!Syntax::s.m_currentSystem->m_allowedBaseTypes.contains(t.m_value)) {
                ...
                //            ErrorHandler::e.Error("Unknown type : "+t.m_value,
                //            m_currentToken.m_lineNumber);
                QString similarSymbol =
                    m_symTab->findSimilarSymbol(val, 65, 2, m_procedures.keys());
                QString em = "Unknown or illegal type : '" +
                             val.remove(m_symTab->m_gPrefix) + "'. ";
                if (similarSymbol != "") {
                    em += "Did you mean '<font color=\"#A080FF\">" + similarSymbol +
                          "</font>'?<br>";
                }
...
*/
        ...
        QString similarSymbol =
            m_symTab->findSimilarSymbol(val, 65, 2, m_procedures.keys());
        QString em =
            "Unknown or illegal type : '" + val.remove(m_symTab->m_gPrefix) + "'. ";
        if (similarSymbol != "") {
            em += "Did you mean '<font color=\"#A080FF\">" + similarSymbol +
                  "</font>'?<br>";
        }
        ...
        ErrorHandler::e.Error(em, m_currentToken.m_lineNumber);
        //      qDebug() <<m_symTab->m_records.keys()
        //      <<m_symTab->m_gPrefix<<t.m_value;
        throw fe;
    }
}
...
void Parser::RemoveUnusedProcedures() {
  ...
  for (QSharedPointer<Node> n:m_proceduresOnly) {
    ...
    for (int i = 0; i < 6; i++)
        for (QSharedPointer<Node> n : m_proceduresOnly) {
            QSharedPointer<NodeProcedureDecl> np =
                qSharedPointerDynamicCast<NodeProcedureDecl>(n);
            ...
            if (isUsed) {
                ...
                if (!np->m_isUsedBy.contains("main") &&
                    !m_doNotRemoveMethods.contains(np->m_procName)) {
...
void Parser::RemoveUnusedSymbols(QSharedPointer<NodeProgram> root) {
    ...
    for (QSharedPointer<Node> n : root->m_NodeBlock->m_decl) {
        ...
        if (nv != nullptr) {
            ...
            if (m_symTab->m_symbols.contains(val) &&
                !m_doNotRemoveMethods.contains(val)) {

                QSharedPointer<Symbol> s =
                    m_symTab->m_symbols[val]; // m_symTab->Lookup(,0);
                //                qDebug() << s->m_type;
...
void Parser::HandlePreprocessorInParsing() {
    ...
    if (m_pass >= PASS_CODE) {
        ...
        if (m_currentToken.m_value == "raisewarning") {
            Eat();
            if (!m_ignoreAll)
                ErrorHandler::e.Warning(m_currentToken.m_value,
                                        m_currentToken.m_lineNumber);
            //        Eat();
            Eat();
            return;
        ...
        if (m_currentToken.m_value == "raiseerror") {
            Eat();
            if (!m_ignoreAll)
                ErrorHandler::e.Error(m_currentToken.m_value,
                                      m_currentToken.m_lineNumber);
        ...
        if (m_currentToken.m_value == "startassembler" ||
            m_currentToken.m_value == "endassembler") {
            Eat();
            Eat();
            return;
        }
    ...
    if (!m_ignoreAll) {
        ...
        if (m_currentToken.m_value == "else") {
            //        qDebug() << "Start with ELSE : " << m_lastIfdef.last() <<
            //        m_lastKey.last();
    ...
    if (m_currentToken.m_value == "endblock") {
        ...
        if (Node::m_staticBlockInfo.m_blockID == -1) {
            ErrorHandler::e.Error("Cannot end a block that hasn't been started.",
                                  m_currentToken.m_lineNumber);
        }
        m_pass = PASS_OTHER;
        Eat();
        //        if (i == PASS_CODE)
        //          qDebug() << "HandleCurrent Endblock " <<
        //          Node::m_staticBlockInfo.m_blockPos;
        m_pass = i;
    ...
    qDebug() << Node::m_staticBlockInfo.m_blockName;*/
        ...
        //  if (i == PASS_CODE)
        //    qDebug() << "HandleCurrent StartBlock " <<
        //    Node::m_staticBlockInfo.m_blockPos;
...
void Parser::StripWhiteSpaceBeforeParenthesis() {
    m_lexer->m_text =
        m_lexer->m_text.replace(QRegularExpression("\\s*(\\()"), "\\1");
    ...
    while (m_currentToken.m_type != TokenType::TEOF) {
        if (m_currentToken.m_type == TokenType::PREPROCESSOR) {
            if (m_currentToken.m_value.toLower() == "include") {
                ...
                m_lexer->m_includeFiles.append(FilePart(
                    name, ln, ln + count, ln - m_acc, ln + count - m_acc, count));
                m_acc -= count - 1;
                done = false;
                Eat(TokenType::STRING);
            }
...
QSharedPointer<Node> Parser::Variable(bool isSubVar) {
    ...
    if (m_currentToken.m_value == Syntax::s.thisName) {
        m_currentToken.m_value =
            m_symTab->m_currentClass + "_" + m_currentToken.m_value;
        m_isClassReference = m_currentToken.m_isReference;
    }

    if (m_currentToken.m_type != TokenType::STRING &&
        m_currentToken.m_type != TokenType::CSTRING)
    ...
    if (!isSubVar && !isRegister &&
        m_symTab->m_constants.contains(m_currentToken.m_value.toUpper())) {
        QSharedPointer<Symbol> s =
            m_symTab->m_constants[m_currentToken.m_value.toUpper()];
        isConstant = true;
        QString val = m_currentToken.m_value.toUpper();

        //        qDebug() << "PARSER looking for " << m_currentToken.m_value << " ,
        //        found symbol: " << s->m_name << " with value " <<
        //        s->m_value->m_fVal << " of type " << s->m_type;
        ...
        if (s->m_type == "ADDRESS")
            m_currentToken.m_type = TokenType::ADDRESS;
        if (s->m_type == "LONG")
            m_currentToken.m_type = TokenType::LONG;
        if (s->m_type == "INTEGER")
            m_currentToken.m_type = TokenType::INTEGER;
        if (s->m_type == "WORD")
            m_currentToken.m_type = TokenType::INTEGER;
        if (s->m_type == "BYTE")
            m_currentToken.m_type = TokenType::BYTE;
        if (s->m_type == "BOOLEAN")
            m_currentToken.m_type = TokenType::BOOLEAN;
        if (s->m_type == "STRING")
        ...
        if (m_currentToken.m_type == TokenType::LBRACKET) {
            ...
            // qDebug() << "PARSER " <<org;
            //            if (isSubVar &&
            //            !m_symTab->m_gPrefix.startsWith("localVariable"))
            m_symTab->m_gPrefix = "";
            expr = Expr();
            expr->m_isIndex = true;
      ...
      */
        ...
        if (t.m_type == TokenType::ADDRESS && expr != nullptr) {
            ...
            QSharedPointer<NodeVar> nv =
                QSharedPointer<NodeVar>(new NodeVar(t, expr));
            nv->m_subNode = subVar;
            ...
            //            qDebug()  << "PARSER::VARIABLE " <<s->m_name <<
            //            n->HexValue() << s->m_value->m_fVal << n->isAddress();
        }
        if (val == "TRUE" ||
            val == "FALSE") // setting the boolean flag, used for comparisone
            n->m_isBoolean = true;
        ...
        //        qDebug() << "PREFIX " <<
        //        m_symTab->m_globalList.contains(t.m_value) << t.m_value <<
        //        m_symTab->m_gPrefix+t.m_value ;
        ...
        // qDebug() << "PARSER2: " <<t.m_value << m_symTab->m_gPrefix
        // <<m_symTab->m_globalList.contains(t.m_value) << m_symTab->m_gPrefix;
        //      qDebug() <<
        ...
        */
            ...
            */
            ...
            qSharedPointerDynamicCast<NodeVar>(n)->m_subNode = subVar;
            QSharedPointer<Symbol> s =
                m_symTab->Lookup(qSharedPointerDynamicCast<NodeVar>(n)->value,
                                                        m_currentToken.m_lineNumber);
            qSharedPointerDynamicCast<NodeVar>(n)->value =
                s->m_name; // make sure that variable name is proper

            if (expr != nullptr && t.m_isReference &&
                !m_symTab->m_records.contains(s->getEndType())) {
                ...
                if (size != 1) {
                    ...
                    if (size == 4)
                        s = 2; // Shift value
                    tn.m_intVal = s;
                    expr2 = QSharedPointer<NodeBinOP>(new NodeBinOP(
                        expr, tb2, QSharedPointer<NodeNumber>(new NodeNumber(tn, s))));
                }
                QSharedPointer<Node> nb =
                    QSharedPointer<NodeBinOP>(new NodeBinOP(n, tb, expr2));
                n = nb;
            }
        }
    }
  ...
  if (nv->m_subNode!=nullptr &&
  (qSharedPointerDynamicCast<NodeVar>nv->m_subNode)->m_expr!=nullptr) nv->m_expr
  = (qSharedPointerDynamicCast<NodeVar>nv->m_subNode)->m_expr;
*/
    ...
    if (nv != nullptr) {
        ...
        QSharedPointer<Symbol> s =
            m_symTab->Lookup(nv->value, m_currentToken.m_lineNumber);
        ...
        //        qDebug
        if (s == nullptr) {
            ErrorHandler::e.Error("Could not find variable/procedure : " + nv->value +
                                  ". Are you sure it is defined?");
        }
        //        qDebug() << nv->value<<s->m_type;
        if (!(s->m_type.toUpper() == "ARRAY" || s->m_type.toUpper() == "POINTER" ||
              s->m_type.toUpper() == "STRING" || s->m_type.toUpper() == "CSTRING" ||
              s->m_type.toUpper() == "INCBIN" || s->m_type.toUpper() == "ADDRESS" ||
              isConstant) &&
            nv->m_expr != nullptr)
            ErrorHandler::e.Error("Variable '<b>" + nv->value +
                                      "</b>' is neither a pointer nor an array.",
                                  nv->m_op.m_lineNumber);
    }
    return ApplyClassVariable(n);
}

QSharedPointer<Node>
Parser::SubVariable(QString parent, QSharedPointer<Node> parentExpr = nullptr) {
    QString fix = m_symTab->m_gPrefix;
    //   qDebug() << "SUBVAR ORG " <<fix << parent << m_symTab->m_symbols.keys();
    QSharedPointer<Symbol> s =
        m_symTab->Lookup(parent, m_currentToken.m_lineNumber);
    ...
    if (m_symTab->m_records.contains(recordType))
        if (m_symTab->m_records[recordType]->m_isClass) {
            ...
            if (m_procedures.contains(procName)) {
                ...
                m_symTab->m_gPrefix = "";
                //           qDebug() << "Testing for procname " <<procName<<
                //           parent<<parentExpr<<m_symTab->m_gPrefix;
...
QVector<QSharedPointer<Node>> Parser::Record(QString name) {
    ...
    if (isClass && !Syntax::s.m_currentSystem->m_allowClasses)
        ErrorHandler::e.Error("TRSE doesn't support classes on this CPU yet",
                              m_currentToken.m_lineNumber);

    Eat();
    //    SymbolTable
    QSharedPointer<SymbolTable> record =
        QSharedPointer<SymbolTable>(new SymbolTable());
    record->m_symbols.clear();
    /*    if
     (m_doNotPrefixSymbols.contains(name.remove(0,m_symTab->m_currentUnit.count())))
    ...
    if (isClass) {
        ...
        m_symTab->m_gPrefix = "";
        auto s = QSharedPointer<Symbol>(
            new Symbol(name + "_" + Syntax::s.thisName, name));
    ...
    while (m_currentToken.m_type != TokenType::END) {
        ...
        if (!isClass)
            if (m_currentToken.m_type == TokenType::PROCEDURE ||
                ...
                ErrorHandler::e.Error("Procedures and funtions can only be declared in "
                                      "classes, not in records",
                                      m_currentToken.m_lineNumber);
            }
        ...
        if (isClass) {
            ...
            if (m_currentToken.m_type == TokenType::PROCEDURE ||
                ...
                for (auto &n : procs) {
                    ...
                    auto type =
                        QSharedPointer<NodeVarType>(new NodeVarType(typeToken, ""));
                    ...
                    auto paramSelf =
                        QSharedPointer<NodeVarDecl>(new NodeVarDecl(var, type));
                    ...
                    first = false;
                    //                 qDebug() << "HERE" << name <<
                    //                 paramSelf<<p->m_paramDecl.count();

                    //                   p->m_paramDecl.insert(0,paramSelf);
                }
            }
        }
        if (m_currentToken.m_type != TokenType::END &&
            m_currentToken.m_type != TokenType::PROCEDURE &&
            m_currentToken.m_type != TokenType::FUNCTION) {
            //            qDebug() << "A "<<m_symTab->m_gPrefix;
            QVector<QSharedPointer<Node>> vars = VariableDeclarations("");
            for (QSharedPointer<Node> n : vars) {
                QSharedPointer<NodeVarDecl> nv =
                    qSharedPointerDynamicCast<NodeVarDecl>(n);
                QSharedPointer<NodeVarType> typ =
                ...
                QSharedPointer<Symbol> s =
                    QSharedPointer<Symbol>(new Symbol(var->value, typ->value));
    ...
    //    qDebug() << "************ PARSER Calculating size "<<name
    //    <<record->getSize();;
...
QSharedPointer<Node>
Parser::AssignStatementBetweenObjects(QSharedPointer<Node> left,
    ...
    if (var != nullptr && var->m_subNode == nullptr && var->isPureObject) {
        QString t1, t2;
        //        qDebug() << "HERE " << t2
        //        <<var->value<<var->isRecord(m_symTab,t1)<<t1;
        if (var->isRecord(m_symTab,
                          t1)) { // && m_symTab->m_records[t1]->m_isClass) {

            if (!right->isRecord(m_symTab, t2) || (t1 != t2))
                ErrorHandler::e.Error("Can only assign an object of a record/class to "
                                      "another object of the same type",
                                      left->m_op.m_lineNumber);
...
//                left.m_ex
//                QSharedPointer<NodeAssign> na = QSharedPointer<NodeAssign>(new
NodeAssign(left, t, right));
...
*/
            ...
            if (Syntax::s.m_currentSystem->m_processor == AbstractSystem::MOS6502)
                params << right << NodeFactory::CreateNumber(m_currentToken, 0) << left
                       << NodeFactory::CreateNumber(m_currentToken, size);
            if (Syntax::s.m_currentSystem->m_processor == AbstractSystem::Z80)
                params << right << left
                       << NodeFactory::CreateNumber(m_currentToken, size);
            return QSharedPointer<NodeBuiltinMethod>(new NodeBuiltinMethod(
                "memcpyunroll", params, &Syntax::s.builtInFunctions["memcpyunroll"]));
...
void Parser::VerifyNotInClassAssignTypespec() {
    if (m_symTab->m_currentClass != "")
        ErrorHandler::e.Error(
            "Class variables cannot have default values. These must be set "
            "manually or in a constructor (which is currently not yet implemented)",
            m_currentToken.m_lineNumber);
}
...
QSharedPointer<Node> Parser::AssignStatement() {
    ...
    if (t.m_isReference)
        ErrorHandler::e.Error("Assignment '" + t.m_value +
    ...
    if (m_currentToken.m_type != TokenType::ASSIGN &&
        m_currentToken.m_type != TokenType::ASSIGNOP) {
        //        qDebug() << m_currentToken;
        // First, check if similar procedure exists
        QString val = t.m_value;
        QString em = "Could not find procedure '<font color=\"#FF8000\">" + val +
                     "</font>'<br>";
        //        QString similarSymbol =
        //        m_symTab->findSimilarSymbol(val,70,2,m_procedures.keys());
        QString similarSymbol = "";
        ...
        ErrorHandler::e.Error("Error assigning variable <b>'" + t.m_value +
                                  "'</b>, did you forget a colon or mistype? "
                                  "Syntax should be: <b>'a := b;'</b>.",
                              token.m_lineNumber);
    }
    ...
    if (m_currentToken.m_type == TokenType::ASSIGNOP) {
        ...
        QSharedPointer<NodeBinOP> binop =
            QSharedPointer<NodeBinOP>(new NodeBinOP(left, op, right));

        QSharedPointer<NodeAssign> na =
            QSharedPointer<NodeAssign>(new NodeAssign(left, t, binop));
  ...
  */
    QSharedPointer<NodeAssign> na =
        QSharedPointer<NodeAssign>(new NodeAssign(left, t, right));
...
QSharedPointer<Node> Parser::Statement() {
    ...
    */
        ...
        node = ForLoop(false);
    } else if (m_currentToken.m_type == TokenType::BREAK ||
               m_currentToken.m_type == TokenType::CONTINUE ||
               m_currentToken.m_type == TokenType::RETURN) {
        node = QSharedPointer<NodeControlStatement>(
            new NodeControlStatement(m_currentToken));
        Eat();
        if (m_currentToken.m_type == TokenType::LPAREN) {
            Eat(TokenType::LPAREN);
            Eat(TokenType::RPAREN);
    ...
    if (node == nullptr)
        ErrorHandler::e.Error("Node is nullpointer. Should not happen. Contact "
                              "leuat@irio.co.uk and slap him.",
                              0);
...
QSharedPointer<Node> Parser::Case() {
    QSharedPointer<NodeCase> n =
        QSharedPointer<NodeCase>(new NodeCase(m_currentToken));
    Eat(); // Eat "case"
    n->m_variable = Expr();
    /*    n->m_variable = qSharedPointerDynamicCast<NodeVar>(Variable());
  if (n->m_variable==nullptr)
      ErrorHandler::e.Error("Case statements only work with variables.",
  m_currentToken.m_lineNumber);
      */
    ...
    Eat(TokenType::OF);
    while (m_currentToken.m_type != TokenType::END &&
           m_currentToken.m_type != TokenType::ELSE) {
        QSharedPointer<Node> expr = Expr();
        Eat(TokenType::COLON);
...
QSharedPointer<Node> Parser::BinaryClause() {
    if (m_currentToken.m_type == TokenType::LPAREN && !nextIsExpr()) {
        ...
        Eat(TokenType::RPAREN);
        return QSharedPointer<NodeBinaryClause>(
            new NodeBinaryClause(logical, a, b));
    }
    // qDebug() << "***** BEFORE EXPR";
    ...
    if (m_currentToken.m_type == TokenType::RPAREN ||
        ...
        if (!(comparetoken.m_type == TokenType::EQUALS ||
              comparetoken.m_type == TokenType::NOTEQUALS ||
              comparetoken.m_type == TokenType::GREATER ||
              comparetoken.m_type == TokenType::LESS ||
              comparetoken.m_type == TokenType::GREATEREQUAL ||
              comparetoken.m_type == TokenType::LESSEQUAL)) {
            ErrorHandler::e.Error("Unknown compare type : '" + comparetoken.m_value +
                                      "'. Did you mean '=' or '>' etc?",
                                  comparetoken.m_lineNumber);
        }
    ...
    return QSharedPointer<NodeBinaryClause>(
        new NodeBinaryClause(comparetoken, a, b));
}
...
bool Parser::isClass(Token &t) {
  ...
  if (m_symTab->m_records.contains(t.m_value))
      qDebug() <<t.m_value
  <<isRecord(t)<<m_symTab->m_records[t.m_value]->m_isClass;
*/
...
QString
Parser::CompareAndWashOverloadedProcedure(QString procName,
                                          QVector<QSharedPointer<Node>> decls) {
    int cnt = 0;
    for (auto pr : m_procedures) {
        auto p = qSharedPointerDynamicCast<NodeProcedureDecl>(pr);
        if (p->m_procName == procName ||
            p->m_procName.startsWith(procName + m_override)) {
            if (CompareDeclerations(decls, p->m_paramDecl) == 0)
                ErrorHandler::e.Error(
                    "Procedure '" + WashVariableName(procName) +
                        "' is already declared with identical parameters. ",
                    m_currentToken.m_lineNumber);
            cnt++;
        }
    }
...
int Parser::CompareDeclerations(QVector<QSharedPointer<Node>> aa,
                                QVector<QSharedPointer<Node>> bb) {
    ...
    for (int i = 0; i < aa.count(); i++) {
        ...
        if (t1->m_op.m_value != t2->m_op.m_value)
            cnt++;
        if (t1->m_flags.count() != t2->m_flags.count())
            cnt++;
        for (auto &s : t1->m_flags)
            if (!t2->m_flags.contains(s) && s != "global")
                cnt++;
    }
    return cnt;
}

int Parser::CompareDeclerationsVsParameters(QVector<QSharedPointer<Node>> aa,
                                            QVector<QSharedPointer<Node>> bb) {
    if (aa.count() != bb.count())
        return -1;
    ...
    for (int i = 0; i < aa.count(); i++) {
        ...
        if (p1 == "BYTE" || p1 == "INTEGER" || p1 == "LONG") {
            if (p2 == "INTEGER_CONST" || p2 == "ADDRESS") {
                if (t1->m_flags.contains("pure_variable"))
                    cnt++;
                continue;
            }
            if (p2 == "ID") {
                // Lookup
                auto s =
                    m_symTab->Lookup(bb[i]->m_op.m_value, m_currentToken.m_lineNumber);
                if (s == nullptr) {
                    //                  qDebug() << "OISANN NULLPTR " <<
                    //                  bb[i]->m_op.m_value;
                    // exit(1);
                    return -1;
                }
                if (s->m_type != p1)
                    cnt++;
                if (t1->m_flags.contains("pure_number"))
                    cnt++;
        ...
        if (p1 == "POINTER") {
            if (p2 == "INTEGER_CONST" || p2 == "POINTER" || p2 == "ID")
                continue;
            if (t1->isPure())
                cnt++;
        }
        ...
        //        else
        // qDebug() << "PARSER COMPARE "<< t1->m_op.m_value <<
        // TokenType::getType(bb[i]->m_op.m_type);
        //      if (t1->m_op.m_value != TokenType::getType(bb[i]->m_op.m_type))
        //      cnt++;
    }
    return cnt;
}
...
QSharedPointer<Node> Parser::Conditional(bool isWhileLoop) {
    ...
    if (m_currentToken.m_type == TokenType::THEN ||
        m_currentToken.m_type == TokenType::DO)
        Eat(m_currentToken.m_type);
    else {
    ...
    if (m_currentToken.m_type == TokenType::PERHAPS) {
        ...
        if (m_isFirstPerhaps) {
            ...
            m_procedures[name] = QSharedPointer<NodeProcedureDecl>(
                new NodeProcedureDecl(Token(TokenType::PROCEDURE, name), name));
    ...
    if (isPerhaps) {
        auto left = NodeFactory::CreateBuiltin(m_currentToken, "random",
                                               QVector<QSharedPointer<Node>>());
        QDate date = QDateTime::currentDateTime().date();
        float t = (date.dayOfWeek() + m_perhapsShift++) / 7.0 * 2 * 3.14159265;
        int testVal = 64 + 128 * (cos(t) + 1) / 2;

        auto randCond = NodeFactory::CreateSingleConditional(
            m_currentToken, TokenType::GREATER, false, left,
            NodeFactory::CreateNumber(m_currentToken, testVal), block);
        block = randCond;
    }
    m_isInConditional = false;

    return QSharedPointer<NodeConditional>(
        new NodeConditional(t, forcePage, clause, block, isWhileLoop, nodeElse));
}
...
QVector<QSharedPointer<Node>> Parser::StatementList() {
    ...
    if (m_currentToken.m_type == TokenType::ID)
        //        ErrorHandler::e.Error("Parser::Statementlist SYNTAX ERROR : Token
        //        should not be ID", m_currentToken.m_lineNumber);
        ErrorHandler::e.Error(
...
QSharedPointer<Node> Parser::CompoundStatement() {
    ...
    Eat(TokenType::END);
    QSharedPointer<NodeCompound> root =
...
QSharedPointer<Node> Parser::Program(QString param) {
    ...
    //    QSharedPointer<NodeVar> varNode =
    //    qSharedPointerDynamicCast<NodeVar>Variable();
    ...
    if (m_isTRU) {
        ...
        if (s_usedTRUNames.contains(progName))
            ErrorHandler::e.Error("TRU '" + progName + "' is already defined! ",
                                  m_currentToken.m_lineNumber);
    ...
    QSharedPointer<NodeProgram> program =
        QSharedPointer<NodeProgram>(new NodeProgram(progName, param, block));
    //    if (block!=nullptr)
...
QSharedPointer<Node> Parser::Factor() {
    ...
    if (m_currentToken.m_type == TokenType::LENGTH) {
        ...
        QSharedPointer<Symbol> s =
            m_symTab->Lookup(varName, m_currentToken.m_lineNumber);
        if (s == nullptr) {
        ...
        if (s->m_type.toLower() == "array") {
            //          qDebug() << TokenType::getType(s->m_arrayType)
            //          <<Syntax::s.m_currentSystem->getPointerSize();;
            if (s->m_arrayType == TokenType::POINTER)
                t.m_intVal /= Syntax::s.m_currentSystem->getPointerSize();
            else if (s->m_arrayType == TokenType::INTEGER)
                t.m_intVal /= 2;
            else if (s->m_arrayType == TokenType::LONG)
                t.m_intVal /= 4;
        }

        //        qDebug() << "PARSER HERE " <<t.m_intVal<<s->m_value->m_strVal<<
        //        s->m_size<<s->m_arrayTypeText<<s->m_type;
    ...
    if (m_currentToken.m_type == TokenType::CSTRING) {
        ...
        if (cs == nullptr)
            ErrorHandler::e.Error("Can only cast strings to cstrings.",
                                  m_currentToken.m_lineNumber);
    ...
    if (t.m_type == TokenType::INTEGER_CONST ||
        t.m_type == TokenType::REAL_CONST || t.m_type == TokenType::ADDRESS) {
    ...
    if (t.m_type == TokenType::ID) {
        ...
        if (p != nullptr) {
            if (!p->m_procedure->m_isFunction && p->m_procedure->m_type == 0)
                ErrorHandler::e.Warning(
                    "Using procedures as functions can result in unpredicted "
                    "behaviour. Please convert your procedure '" +
                    p->m_procedure->m_procName +
                    "' into a function in order to avoid potential problems with the "
                    "return value. ");
        }
        if (node != nullptr) {
            return node;
...
QSharedPointer<Node> Parser::RepeatUntil() {
    ...
    QSharedPointer<NodeBlock> block =
        QSharedPointer<NodeBlock>(new NodeBlock(t, decl, root));

    //    qDebug() << "from begin block : " +m_currentToken.getType();
    //    NodeRepeatUntil(Token op, int forcePage, QSharedPointer<Node> cond,
...
QSharedPointer<Node> Parser::Term() {
    ...
    while (m_currentToken.m_type == TokenType::Type::MUL ||
           ...
           m_currentToken.m_type == TokenType::Type::BITAND ||
           m_currentToken.m_type == TokenType::Type::BITOR ||
           m_currentToken.m_type == TokenType::Type::SHR ||
           m_currentToken.m_type == TokenType::Type::SHL ||
...
void Parser::PreprocessSingle() {
    ...
    if (m_currentToken.m_value.toLower() == "copyfile") {
        ...
        if (!QFile::exists(m_currentDir + src))
            ErrorHandler::e.Error("Could not find file to copy: " + src,
                                  m_currentToken.m_lineNumber);
    ...
    if (m_currentToken.m_value.toLower() == "addcpmheader") {
        ...
        Eat();
        if (!QFile::exists(m_currentDir + src))
            ErrorHandler::e.Error("Could not find file to add to header: " + src,
                                  m_currentToken.m_lineNumber);
    ...
    } else if (m_currentToken.m_value.toLower() == "addemulatorparam") {
        Eat(TokenType::PREPROCESSOR);
    ...
    if (m_currentToken.m_value.toLower() == "setvalue") {
    ...
    } else if (m_currentToken.m_value == "generate_unrolled_asm1") {
        ...
        QString name = m_currentToken.m_value;
        bool ok;
        m_userBlocks.append(QSharedPointer<MemoryBlock>(new MemoryBlock(
            Util::NumberFromStringHex(from), Util::NumberFromStringHex(to),
        ...
        Syntax::s.m_currentSystem->m_projectIni->setString("vic_memory_config",
                                                           m_vicMemoryConfig);
        Syntax::s.Reload();
        ...
        if (!QFile::exists(m_currentDir + "/" + requiredFile))
            ErrorHandler::e.Error("The following file is required for compilation: "
                                  "<font color=\"#FF80A0\">'" +
                                      requiredFile +
                                      "'</font>.<br><font color=\"#FFB060\">" +
                                      message + "</font>",
                                  m_currentToken.m_lineNumber);
    ...
    /*  else if (m_currentToken.m_value.toLower() =="error") {
                Eat(TokenType::PREPROCESSOR);
                ErrorHandler::e.Error("Error from preprocessor3: "
     +m_currentToken.m_value);
...
void Parser::PreprocessAll() {
    ...
    while (m_currentToken.m_type != TokenType::TEOF) {
        ...
        //      if (m_pass==1)
        //      emit
        //      EmitTick("&"+QString::number((int)(100*m_lexer->m_pos/(float)m_lexer->m_text.size())));
...
void Parser::PreprocessMacros() {
    ...
    while (m_currentToken.m_type != TokenType::TEOF) {
        ...
        if (m_currentToken.m_type == TokenType::PREPROCESSOR) {
            // qDebug() << m_currentToken.m_value
            // <<m_macros.contains(m_currentToken.m_value.toLower());
            if (m_macros.contains(m_currentToken.m_value.toLower())) {
...
QSharedPointer<Node> Parser::Parse(bool removeUnusedDecls, QString param,
                                   QString globalDefines, bool useLocals) {
  ...
  PreprocessMacros();*/
    ...
    if (m_projectIni->getdouble("save_unrolled_macros") == 1 &&
        m_containsMacros) {
        qDebug().noquote() << m_lexer->m_text;
        Util::SaveTextFile(m_currentDir + "/macro_unrolled/" +
                               QFileInfo(m_currentFileShort).baseName() + ".ras",
                           m_lexer->m_text);
    }
...
|_|   \__,_|_|  |___/\___|_|
                              ...
                              */
    QSharedPointer<NodeProgram> root =
        qSharedPointerDynamicCast<NodeProgram>(Program(param));
    ...
    for (QString s : m_procedures.keys())
        if (qSharedPointerDynamicCast<NodeProcedureDecl>(m_procedures[s])
                ->m_block == nullptr) {
    ...
    // for (QString s: m_procedures.keys())
    //   if
    //   (((QSharedPointer<NodeProcedureDecl>)m_procedures[s])->m_block!=nullptr)
    for (QSharedPointer<Node> &n : m_proceduresOnly)
...
QSharedPointer<Node>
Parser::FindProcedure(bool &isAssign,
                      QSharedPointer<Node> parentExpr = nullptr) {
    isAssign = false;
    ...
    if (m_isTRU)
        triple =
            m_procedures.contains(m_symTab->m_currentUnit + m_currentToken.m_value);
    ...
    if (m_procedures.contains(m_symTab->m_gPrefix + m_currentToken.m_value) ||
        dual || triple) {
        QString procName =
            m_symTab->m_gPrefix + m_currentToken.m_value; // fix prefix

        if (triple)
            procName = m_symTab->m_currentUnit + m_currentToken.m_value;
        if (dual)
            procName = m_currentToken.m_value;
        Token t = m_currentToken;
        Eat(TokenType::ID);

        // Return value from "Function"
        if (m_currentToken.m_type ==
            TokenType::ASSIGN) { // IS function ASSIGN myProc:= ... - so return
            // value
            // ASSIGN procedure for return value
            //            qDebug() << procName << m_inCurrentProcedure;
            if (procName != m_inCurrentProcedure)
                ErrorHandler::e.Error("Function return name needs to be identical the "
                                      "name of the function itself.",
                                      m_currentToken.m_lineNumber);
            Eat();
            auto node =
                qSharedPointerDynamicCast<NodeProcedureDecl>(m_procedures[procName]);
            if (!node->m_isFunction)
                ErrorHandler::e.Error(
                    "Only functions can have intrinsic return values.",
                    m_currentToken.m_lineNumber);
            if (node->m_returnValue != nullptr)
                ErrorHandler::e.Error("You can only set the return value once in the "
        ...
        if (m_currentToken.m_type != TokenType::SEMI &&
            m_currentToken.m_type != TokenType::RPAREN) {
            ...
            while (m_currentToken.m_type != TokenType::RPAREN &&
                ...
                if (m_currentToken.m_type == TokenType::COMMA) {
                    Eat(TokenType::COMMA);
                } else if (m_currentToken.m_type == TokenType::RPAREN) {

                } else {
                    ErrorHandler::e.Error(
                        "Uknown token '" + m_currentToken.m_value +
                            "'. Please separate your parametes by commas. ",
                        m_currentToken.m_lineNumber);
                }
                // if (m_currentToken.m_type==TokenType::SEMI)
                //     ErrorHandler::e.Error("Syntax errror",
                //     m_currentToken.m_lineNumber);
            }
            //        qDebug() << "Searching for :" << procName << " in " <<
            //        m_procedures.keys();
            if (!m_procedures.contains(procName))
                ErrorHandler::e.Error("Could not find procedure :" + procName,
                                      m_currentToken.m_lineNumber);

            Eat(TokenType::RPAREN);
        }
        //        QSharedPointer<NodeProcedureDecl> p =
        //        qSharedPointerDynamicCast<NodeProcedureDecl>(m_procedures[procName]);
    ...
    //        p =
        ...
        if (m_addInitialReferenceToProcedureCall != "") {
            ...
            QSharedPointer<Symbol> s =
                m_symTab->Lookup(classRef.m_value, m_currentToken.m_lineNumber);
...
//             qDebug() << nv->m_expr->getValue(nullptr);
    ...
    // qDebug() << m_currentToken.getType() << " with value " <<
    // m_currentToken.m_value;
    return nullptr;
}
...
QSharedPointer<Node> Parser::Block(bool useOwnSymTab, QString blockName) {
    ...
    /*    if (m_currentToken.m_type!=TokenType::VAR  &&
     m_currentToken.m_type!=TokenType::BEGIN) return nullptr;
*/
    // Main block
    ...
    QSharedPointer<NodeBlock> bl = QSharedPointer<NodeBlock>(
        new NodeBlock(m_currentToken, decl, statements, useOwnSymTab));
    bl->m_op.m_lineNumber = pos;
    return bl;
}

QSharedPointer<Node> Parser::BlockNoCompound(bool useOwnSymTab,
                                             QString blockName) {
    ...
    QVector<QSharedPointer<Node>> decl =
        Declarations(useOwnSymTab, blockName, false);
    int pos = m_currentToken.m_lineNumber;
    QSharedPointer<NodeBlock> bl = QSharedPointer<NodeBlock>(
        new NodeBlock(m_currentToken, decl, nullptr, useOwnSymTab));
    bl->m_op.m_lineNumber = pos;
    return bl;
}
    ...
    if (m_currentToken.m_type == TokenType::LPAREN) {
        ...
        while (m_currentToken.m_type == TokenType::ID) {
            ...
            if (m_currentToken.m_type == TokenType::COMMA ||
                ...
                ErrorHandler::e.Error("Unexpected token '" + m_currentToken.m_value +
                                          "'",
                                      m_currentToken.m_lineNumber);
        }
    }
...
QSharedPointer<Node> Parser::ForLoop(bool inclusive) {
    ...
    //    qDebug() << m_currentToken.getType();
    //  exit(1);
    return QSharedPointer<NodeForLoop>(new NodeForLoop(
        a, b, block, step, unroll, forcePage, loopType, inclusive));
}
...
QSharedPointer<Node> Parser::String(bool isCString = false) {
    ...
    if (m_currentToken.m_type == TokenType::STRING ||
        m_currentToken.m_type == TokenType::CSTRING) {
        QSharedPointer<NodeString> node = QSharedPointer<NodeString>(
            new NodeString(m_currentToken, QStringList() << m_currentToken.m_value,
                           m_currentToken.m_type == TokenType::CSTRING));
    ...
    if (m_currentToken.m_type != TokenType::LPAREN)
        ErrorHandler::e.Error("String declaration must be single string or "
                              "paranthesis with multi values.",
                              m_currentToken.m_lineNumber);
    ...
    while (m_currentToken.m_type != TokenType::RPAREN) {
        ...
        if (m_currentToken.m_value == "" ||
            m_currentToken.m_type == TokenType::ID ||
            m_currentToken.m_type == TokenType::LPAREN)
            m_currentToken.m_value =
                numID + QString::number(GetParsedInt(TokenType::INTEGER));
...
*/
    ...
    //    qDebug() <<m_currentToken.getType();
    QSharedPointer<NodeString> node = QSharedPointer<NodeString>(
        new NodeString(token, lst, token.m_type == TokenType::CSTRING));
    return node;
}

void Parser::VarDeclarations(QVector<QSharedPointer<Node>> &decl,
                             QString blockName) {
    if (m_currentToken.m_type == TokenType::VAR)
        Eat(TokenType::VAR);
    while (m_currentToken.m_type == TokenType::ID ||
           m_currentToken.m_type == TokenType::CONSTANT ||
           m_currentToken.m_type == TokenType::TYPE) {
        if (m_currentToken.m_type == TokenType::CONSTANT) {
            ConstDeclaration();
        } else if (m_currentToken.m_type == TokenType::TYPE) {
            TypeDeclaration();
        } else {
...
void Parser::ProcDeclarations(QVector<QSharedPointer<Node>> &decl,
                              QString blockName) {
    int type = 0;
    if (m_currentToken.m_type == TokenType::INTERRUPT)
        type = 1;
    if (m_currentToken.m_type == TokenType::WEDGE)
        type = 2;
    bool isFunction = m_currentToken.m_type == TokenType::FUNCTION;
    //   bool isInterrupt=
    //   (m_currentToken.m_type==TokenType::PROCEDURE)?false:true;
    Token tok = m_currentToken;
    Eat(m_currentToken.m_type);
    bool isInline = false;
    bool isRecursive = false;
    QString procName =
        m_procPrefix + m_symTab->m_gPrefix + m_currentToken.m_value;
    ...
    for (auto &v : paramDecl) {
        ...
        if (qSharedPointerDynamicCast<NodeVarType>(nv->m_typeNode)
                ->m_flags.contains("stack")) {
            auto s = m_symTab->Lookup(var->value, m_currentToken.m_lineNumber);
            if (s->m_type.toLower() != "byte") {
                ErrorHandler::e.Error("Stack parameters can currently only be 'byte'",
                                      m_currentToken.m_lineNumber);
            }
    ...
    if (isFunction) {
        ...
        QStringList allowed = QStringList()
                              << "integer" << "byte" << "long" << "boolean";
        if (!(allowed.contains(t->value.toLower()))) {
            ErrorHandler::e.Error("TRSE currently only supports return values of "
                                  "type 'byte', 'integer', 'boolean' and 'long'",
                                  t->m_op.m_lineNumber);
        }
    }
    ...
    QSharedPointer<NodeProcedureDecl> procDecl =
        QSharedPointer<NodeProcedureDecl>(
        new NodeProcedureDecl(tok, procName, paramDecl, block, type));
    //    qDebug() << "Starting new procedure decl block with : "<< procName <<
    //    procDecl->m_blockInfo.m_blockID;
    ...
    if (isInline) {
        for (auto &v : paramDecl) {
            auto var = qSharedPointerDynamicCast<NodeVar>(v);
            auto nv = qSharedPointerDynamicCast<NodeVarDecl>(v);
            if (qSharedPointerDynamicCast<NodeVarType>(nv->m_typeNode)
                    ->m_flags.contains("global")) {
//                ErrorHandler::e.Error("Inline functions cannot use global parameters",
  //                                    tok.m_lineNumber);
            }
            // make sure nodevardecl is toggled as inline
            nv->m_isInline = isInline;
        }
        for (auto &s : m_localSyms) {
            // Make sure symbols are defined as inline
            s->m_isInline = true;
        }
    }
    ...
    if (m_procedures[procName] != nullptr) {
        //  qDebug() << "Procedure already defined: testing for forward "<<procName
        //  <<m_proceduresOnly.count();
        procDecl->m_isUsed = m_procedures[procName]->m_isUsed;
        procDecl->m_isUsedBy = m_procedures[procName]->m_isUsedBy;
        if (qSharedPointerDynamicCast<NodeProcedureDecl>(m_procedures[procName])
                ->m_isForward == false) {
            // ErrorHandler::e.Error("Procedure '"+ WashVariableName(procName) +"' is
            // already declared.", tok.m_lineNumber);
            procName = CompareAndWashOverloadedProcedure(procName, paramDecl);
            isOverload = true;
            //            procName = procName + "_overload_";
        }

        // Make sure that the correct number of parameters + types etc are identical
        // for the forward declared procedure
        if (!isOverload) {
            QSharedPointer<NodeProcedureDecl> existing =
                qSharedPointerDynamicCast<NodeProcedureDecl>(m_procedures[procName]);
            if (existing->m_paramDecl.count() != procDecl->m_paramDecl.count())
                ErrorHandler::e.Error("Forward declared procedure '" +
                                          WashVariableName(procName) +
                                          "' has incorrect number of parameters. ",
                                      tok.m_lineNumber);

            if (!isOverload)
                for (int i = 0; i < existing->m_paramDecl.count(); i++) {
                    QSharedPointer<NodeVarDecl> a =
                        qSharedPointerDynamicCast<NodeVarDecl>(existing->m_paramDecl[i]);
                    QSharedPointer<NodeVarDecl> b =
                        qSharedPointerDynamicCast<NodeVarDecl>(procDecl->m_paramDecl[i]);
                    if (qSharedPointerDynamicCast<NodeVar>(a->m_varNode)->value !=
                        qSharedPointerDynamicCast<NodeVar>(b->m_varNode)->value)
                        ErrorHandler::e.Error(
                            "Forward declared procedure '" + WashVariableName(procName) +
                                "' has incorrect or missing declared parameter '" +
                                WashVariableName(
                                qSharedPointerDynamicCast<NodeVar>(a->m_varNode)
                                                                                                                                                                   ->value) +
                                "'",
                            tok.m_lineNumber);
                    ...
                    if (qSharedPointerDynamicCast<NodeVarType>(a->m_typeNode)->value !=
                        ...
                        ErrorHandler::e.Error(
                            "Forward declared procedure '" + WashVariableName(procName) +
                                "' has incorrect declared parameter type for parameter '" +
                                WashVariableName(
                                qSharedPointerDynamicCast<NodeVar>(a->m_varNode)
                                                                                                                                                                           ->value) +
                                "', should be " +
                                WashVariableName(
                                qSharedPointerDynamicCast<NodeVarType>(a->m_typeNode)
                                                                                                                                                                                                                                                                         ->value),
                            tok.m_lineNumber);
                }
        }
    ...
    if (isFunction)
        qSharedPointerDynamicCast<NodeProcedureDecl>(m_procedures[procName])
            ->m_returnType = funcType;

    // For recursive procedures, it is vital that we forward declare the current
    // procedure
    if (!isForward) {
        if (isAssembler) {
            QSharedPointer<NodeBlock> b = QSharedPointer<NodeBlock>(new NodeBlock(
                m_currentToken, QVector<QSharedPointer<Node>>(), InlineAssembler()));
            block = b;
        } else
    ...
    if (block != nullptr) {
        ...
        for (auto &p : procDecl->m_paramDecl) {
            QSharedPointer<NodeVarDecl> nv =
                qSharedPointerDynamicCast<NodeVarDecl>(p);
            QSharedPointer<NodeVar> v =
                qSharedPointerDynamicCast<NodeVar>(nv->m_varNode);
    ...
    if (block != nullptr && !alreadyForwaredDeclared) {
        bool ok = true;
        for (QSharedPointer<Node> n : m_proceduresOnly) {
            QSharedPointer<NodeProcedureDecl> proc =
                qSharedPointerDynamicCast<NodeProcedureDecl>(n);
        ...
        if (!ok)
            ErrorHandler::e.Error("Procedure '" + procName + "' already defined",
                                  tok.m_lineNumber);
    }
...
QSharedPointer<Node> Parser::ApplyClassVariable(QSharedPointer<Node> var) {
    ...
    QSharedPointer<Symbol> s =
        m_symTab->Lookup(v->value, m_currentToken.m_lineNumber);
    QString t1;
    ...
    if (!(m_symTab->m_records.contains(type) &&
          m_symTab->m_records[type]->m_isClass)) {
            ...
            if (s->m_type != "POINTER")
                if (s->m_arrayType != TokenType::NADA)
                    if (v->m_expr == nullptr)
                        if (!v->isReference())
                            ErrorHandler::e.Error(
                                "Unknown usage of data or array. <font color=\"orange\">Did "
                                "you mean to reference it? (#" +
                                    v->value + ")</font>",
                                m_currentToken.m_lineNumber);
        ...
        if (v->m_expr != nullptr && scale != 1 && !v->m_scaleApplied) {
            v->m_expr = NodeFactory::CreateBinop(
                m_currentToken, TokenType::MUL, v->m_expr,
                NodeFactory::CreateNumber(m_currentToken, scale));
    ...
    if (isArray && v->m_expr == nullptr && !v->isReference()) {
        ErrorHandler::e.Error("Variable '" + v->value +
    ...
    if (size != 0 && v->m_expr != nullptr) {
        ...
        if (v->m_expr->isPureNumeric() && v->m_expr->numValue() == 0) {
            ...
            v->m_expr = NodeFactory::CreateBinop(m_currentToken, TokenType::MUL,
                                                 v->m_expr, nodeClassSize);
    }
    ...
    if (sv != nullptr) {
        ...
        int shiftInternal =
            m_symTab->m_records[type]->getShiftedPositionOfVariable(sv->value, 1);
        // internalshift is the offset of the variable position in the class
        if (shiftInternal != 0)
            //            qDebug() << "internal shift for var "+sv->value+" : Adding
            //            "<<shiftInternal;
            v->m_expr = NodeFactory::CreateBinop(
                m_currentToken, TokenType::PLUS, v->m_expr,
                NodeFactory::CreateNumber(m_currentToken, shiftInternal));
        ...
        if (node != nullptr) {
            //            qDebug() << "Adding :
            //            "<<QString::number(v->m_expr->getValueAsInt(nullptr))<<" to
            //            " <<QString::number(node->getValueAsInt(nullptr)); qDebug()
            //            << "Adding :
            //            "<<QString::number(node->getValueAsInt(nullptr));
            v->m_expr = NodeFactory::CreateBinop(m_currentToken, TokenType::PLUS,
                                                 v->m_expr, node);
        }
        ...
        //      qDebug() << "PARSER  type "<< type
        //      <<v->value<<TokenType::getType(v->m_classvariableType);
    ...
    if (v->m_expr->isPureNumeric())
        v->m_expr =
            NodeFactory::CreateNumber(v->m_op, v->m_expr->getValueAsInt(nullptr));
...
QSharedPointer<NodeProcedureDecl>
Parser::FindCorrectOverridenProcedure(QString procName,
                                      QVector<QSharedPointer<Node>> params) {
    ...
    if (lst.count() == 1)
        return lst
            .first(); // qSharedPointerDynamicCast<NodeProcedureDecl>(m_procedures[procName]);
    for (auto l : lst) {
...
QVector<QSharedPointer<Node>>
Parser::Declarations(bool isMain, QString blockName, bool hasBlock) {
    QVector<QSharedPointer<Node>> decl;
    ...
    while (m_currentToken.m_type == TokenType::PROCEDURE ||
           ...
           m_currentToken.m_type == TokenType::WEDGE ||
           m_currentToken.m_type == TokenType::FUNCTION ||
           m_currentToken.m_type == TokenType::VAR) {
        if (m_currentToken.m_type == TokenType::VAR) {
    ...
    if (m_currentToken.m_type != TokenType::BEGIN && isMain && hasBlock)
        ErrorHandler::e.Error("After declarations, BEGIN is expected",
                              m_currentToken.m_lineNumber);
...
QVector<QSharedPointer<Node>> Parser::ConstDeclaration() {
    ...
    if (type == "") {
        ErrorHandler::e.Error(
            "Unknown or illegal type when defining constant of type: '" +
                m_currentToken.m_value + "' (" + m_currentToken.getType() +
                ")<br>Allowed types are : <b>address, byte, integer.</b> ",
            m_currentToken.m_lineNumber);
    }
    Eat();
    Eat(TokenType::EQUALS);
    ...
    if (m_symTab->exists(name))
        ErrorHandler::e.Error("Symbol '" + name + "' is already defined.",
                              m_currentToken.m_lineNumber);
    m_symTab->m_constants[name.toUpper()] =
        QSharedPointer<Symbol>(new Symbol(name.toUpper(), type.toUpper(), value));
...
QSharedPointer<Node> Parser::TypeDeclaration() {
    ...
    if (m_types.contains(name))
        ErrorHandler::e.Error("Type already defined: " + name,
                              m_currentToken.m_lineNumber);
...
QVector<QSharedPointer<Node>>
Parser::VariableDeclarations(QString blockName, bool isProcedureParams) {
    if (blockName != "")
    ...
    if (m_currentToken.m_type == TokenType::EQUALS) {
        ...
        if (m_currentToken.m_type == TokenType::RECORD ||
            m_currentToken.m_type == TokenType::CLASS) {
    ...
    QStringList preflags =
        getFlags(); // Allow for flags to be defined before the type
    ...
    if (!isGlobal)
        for (QSharedPointer<Symbol> s : syms) {
            if (Syntax::s.m_illegaVariableNames.contains(s->m_name))
                ErrorHandler::e.Error("Illegal variable name '" + s->m_name +
                                          "' on the " +
                                          AbstractSystem::StringFromSystem(
                                          Syntax::s.m_currentSystem->m_system) +
                                          " (name already used in the assembler etc) ",
                                      m_currentToken.m_lineNumber);
    ...
    QSharedPointer<NodeVarType> typeNode = qSharedPointerDynamicCast<NodeVarType>(
        TypeSpec(isProcedureParams, variableNames));
    ...
    if (Syntax::s.m_currentSystem->m_system == AbstractSystem::GAMEBOY ||
        Syntax::s.m_currentSystem->m_system == AbstractSystem::COLECO) {
        // if (typeNode->m_op.m_type==TokenType::POINTER)
        if (typeNode->m_data.count() <= 1 &&
            typeNode->m_op.m_type != TokenType::INCBIN &&
            typeNode->m_op.m_type != TokenType::STRING) {
    ...
    for (auto &s : syms) {
        ...
        if (s->m_type == "STRING" || s->m_type == "CSTRING") {
            ...
            if (isProcedureParams)
                ErrorHandler::e.Error("TRSE does not support string parameters. Please "
                                      "use a byte pointer parameter instead",
                                      m_currentToken.m_lineNumber);
        }
        ...
        //       qDebug() << "FLAGS " << s->m_name << " "
        //       <<s->m_type<<s->m_arrayTypeText;
    ...
    // in case the user tries to use a global variable from another unit, remove
    // the current unit prefix
    if (m_isTRU)
        for (QSharedPointer<Node> n : vars) {
    ...
    if (isGlobal) { // Typecheck that they exist
        for (QSharedPointer<Node> n : vars) {
            ...
            try {
                ...
                fe.message =
                    fe.message + "When using the <font color=\"yellow\">global</font> "
                                          "keyword, the variable must in question must be "
                                          "declared in the global variable scope. ";
                throw fe;
            }
            if (sym->m_type != typeNode->m_op.m_value)
                throw FatalErrorException(
                    "Global parameters must be the same type as defined globally (" +
                        sym->m_type.toLower() +
                        " != " + typeNode->m_op.m_value.toLower() + ") ",
                    m_currentToken.m_lineNumber);
        }
    }
    int add = 0, cur = 0;
    ...
    for (QSharedPointer<Node> n : vars) {
        QSharedPointer<NodeVarDecl> decl =
            QSharedPointer<NodeVarDecl>(new NodeVarDecl(n, typeNode));
        QSharedPointer<NodeVar> v = qSharedPointerDynamicCast<NodeVar>(n);
        if (v->value.startsWith("global_"))
            ErrorHandler::e.Error(
                "Cannot declare variables that start with 'global_' in TRSE, since "
                "this prefix is used internally",
                m_currentToken.m_lineNumber);
        ...
        if (typeNode->m_position != "") { // HAS a fixed position
            // qDebug() << "FX" <<v->value;
            m_symTab->m_extraAtSymbols[v->value] =
                Util::numToHex(Util::NumberFromStringHex(typeNode->m_position) + add);
            add += syms[cur]->m_size;
        ...
        if (typeNode->m_op.m_type == TokenType::INCSID) {
            ...
            if (m_preprocessorDefines.contains("SIDEmulatorLocation")) {
                sidloc = Util::NumberFromStringHex(
                    m_preprocessorDefines["SIDEmulatorLocation"]);
                // qDebug() << sidloc;
            }
            //            exit(1);
...
QSharedPointer<Node> Parser::TypeSpec(bool isInProcedure,
    ...
    // then if you have defined a byte it MUST become an integer due to the
    // alignment requires
    bool forceByteAlignment =
        (Syntax::s.m_currentSystem->m_processor == AbstractSystem::M68000 &&
                               m_isRecord);
    //    qDebug() << "TYPESPEC " <<m_currentToken.getType() << varNames <<
    //    m_symTab->m_currentClass;
...
*/
    if (m_types.contains(t.m_value) ||
        m_types.contains(m_symTab->m_gPrefix + t.m_value)) {
        QString n = t.m_value;
        if (!m_types.contains(t.m_value))
    ...
    if (m_currentToken.m_type == TokenType::INCBIN ||
        m_currentToken.m_type == TokenType::INCSID ||
        m_currentToken.m_type == TokenType::INCNSF) {
        Eat();
        //        qDebug() << Node::m_staticBlockInfo.m_blockPos;
        Eat(TokenType::LPAREN);
        ...
        if (requireSplit) {
            m_splitCounter++;
            QString splitExtension =
                "_split" + QString::number(m_splitCounter) + ".bin";
            QString f = m_currentDir + QDir::separator() + binFile;
            if (!QFile::exists(f))
                ErrorHandler::e.Error("Could not find file for inclusion: " + f,
                                      m_currentToken.m_lineNumber);
        ...
        QSharedPointer<NodeVarType> nt =
            QSharedPointer<NodeVarType>(new NodeVarType(t, binFile, position));
        nt->m_flags = flags;
    ...
    if (m_currentToken.m_type == TokenType::ARRAY) {
        ...
        if (Syntax::s.m_currentSystem->m_processor == AbstractSystem::MOS6502 &&
            ...
            QString ie = "<br><font "
                         "color=\"grey\">Example:<br>anIntegerArray[i]:=#data;<br>"
                         "aPointer := anIntegerArray[i];</font>";
            ErrorHandler::e.Error(
                "Cannot have arrays of (zeropage) pointers on the 6502. Instead, "
                "please use an array of integers, and then assign a pointer to the "
                "(integer) array item." +
                    ie,
                m_currentToken.m_lineNumber);
        }
        ...
        if (m_currentToken.m_type == TokenType::EQUALS) {
            ...
            if (isStringList) {
                ...
                for (auto d : tmp_data) {
                    QString name = "tmp_" + QString::number((rand() % 1000000)) +
                                   "_string" + QString::number(cnt);
                    names.append(name);
                    auto str = qSharedPointerDynamicCast<NodeString>(d);
                    QSharedPointer<NodeVar> var =
                        NodeFactory::CreateVariable(str->m_op, name);
                    Token tt = d->m_op;
                ...
                if (m_currentToken.m_type == TokenType::BUILDTABLE) {
                    ...
                    //                    qDebug() << data;
                } else if (m_currentToken.m_type == TokenType::BUILDSINETABLE) {
                    data = BuildSineTable(count, dataType);
                } else if (m_currentToken.m_type == TokenType::BUILDTABLE2D) {
                    ...
                    while (m_currentToken.m_type != TokenType::RPAREN) {
                        ...
                        if (m_currentToken.m_value != "") {
                            if ((m_symTab->m_symbols.contains(m_currentToken.m_value) &&
                                 (m_symTab->LookupConstants(
                                                                                              m_currentToken.m_value.toUpper()) == nullptr))) {
                                data << m_currentToken.m_value;

                                m_symTab->m_symbols[m_currentToken.m_value]->isUsed = true;
                                m_symTab->m_symbols[m_currentToken.m_value]->isUsedBy
                                    << m_inCurrentProcedure;
                                found = true;
                            }
                            if (m_procedures.contains(m_currentToken.m_value)) {
                                ...
                                m_procedures[m_currentToken.m_value]->m_isUsedBy
                                    << m_inCurrentProcedure;
                                found = true;
                            }
                            //                    qDebug() << "ADDRESS " <<
                        ...
                        if (!found) {
                            QString extra = "";
                            if (Syntax::s.m_currentSystem->m_processor ==
                                AbstractSystem::M68000)
                                extra = "0";
                            data << "$" + extra +
                                        QString::number(
                                        GetParsedInt(dataType),
                                                                  16); // QString::number(m_currentToken.m_intVal);
                        }
                        // data <<
                        // "$0"+QString::number(GetParsedInt(),16);//QString::number(m_currentToken.m_intVal);
                        if (m_currentToken.m_type != TokenType::RPAREN) {
            ...
            if (count != data.count() && count != 0) {
                if (count > data.count()) {
                    ErrorHandler::e.Warning(
                        "Declared array count for variable '" + vars + "' (" +
                            QString::number((int)count) + ") does not match with data (" +
                            QString::number(data.count()) + "). Padding with zeros. ",
                        m_currentToken.m_lineNumber);
                    for (int i = data.count(); i < count; i++)
                        data.append(QString("0"));
                } else
                    ErrorHandler::e.Warning("Declared array count for variable '" + vars +
                                                "' (" + QString::number((int)count) +
                                                ") does not match with data (" +
                                                QString::number(data.count()) +
                                                "). Adjusting array size to fit data. ",
                                            m_currentToken.m_lineNumber);
            }
            //            if (count==0)
            count = data.count();
        ...
        if (m_symTab->m_records.contains(arrayType.m_value)) {
            ...
            if (m_symTab->m_records[arrayType.m_value]->ContainsArrays())
                if (!m_symTab->m_records[arrayType.m_value]->m_isClass)
                    ErrorHandler::e.Error(
                        "You cannot declare an array of records that contain sub-arrays "
                        "due to 6502 limitations. <br>Please remove the sub-array from "
                        "the record type in question : '" +
                            arrayType.m_value + "'.",
                        arrayType.m_lineNumber);
        }
        // m68k alignment for byte arrays
        if (forceByteAlignment && arrayType.m_type == TokenType::BYTE &&
            (t.m_intVal & 1) == 1) {
            t.m_intVal += 1;
            if (data.count() != 0)
                data.append("0");
        ...
        //        qDebug() << "***** " <<t.getType();
        QSharedPointer<NodeVarType> nt = QSharedPointer<NodeVarType>(
            new NodeVarType(t, position, arrayType, data));
        nt->m_flags = flags;
        nt->VerifyFlags(isInProcedure);
    ...
    if (m_currentToken.m_type == TokenType::STRING ||
        ...
        if (m_currentToken.m_type == TokenType::EQUALS) {
            VerifyNotInClassAssignTypespec();
            Eat();
            QSharedPointer<NodeString> str =
                qSharedPointerDynamicCast<NodeString>(String(isCString));
            initData = str->m_val;
        }
        QString position = "";
        if (m_currentToken.m_type == TokenType::AT ||
        ...
        QSharedPointer<NodeVarType> str =
            QSharedPointer<NodeVarType>(new NodeVarType(t, initData));
        str->m_flags = flags;
    ...
    if (m_currentToken.m_isPointer) {
        ...
        if (m_isRecord && !Syntax::s.m_currentSystem->AllowPointerInStructs() &&
            !m_inProcedureVariableDecl)
            ErrorHandler::e.Error(
                "You cannot declare pointers in records/classes on this CPU type. "
                "Please use an integer to store the address instead, and assign a "
                "pointer to it when required.  ",
                m_currentToken.m_lineNumber);

        t.m_type = TokenType::POINTER;
        t.m_value = "POINTER";
        QSharedPointer<NodeVarType> nvt =
            QSharedPointer<NodeVarType>(new NodeVarType(t, ""));
        nvt->m_arrayVarType.m_type = m_currentToken.m_type;

        if (!(m_currentToken.m_type == TokenType::INTEGER ||
              m_currentToken.m_type == TokenType::BYTE ||
              m_currentToken.m_type == TokenType::LONG ||
              isClass(m_currentToken))) {
            ErrorHandler::e.Error(
        ...
        if (m_currentToken.m_type == TokenType::EQUALS) {
            ErrorHandler::e.Error(
                "On the 6502, pointers must be initialized through code. Z80/M68K "
                "pointer initialization not yet implemented in TRSE. ",
                m_currentToken.m_lineNumber);
        }
    ...
    if (m_currentToken.m_type == TokenType::POINTER ||
        m_currentToken.m_type == TokenType::LPOINTER) {
        if (m_isRecord && !Syntax::s.m_currentSystem->AllowPointerInStructs() &&
            !m_inProcedureVariableDecl)
            ErrorHandler::e.Error(
                "You cannot declare pointers in records/classes on this CPU type. "
        ...
        //        qDebug() <<"Parser typespec pointer: "  <<
        //        nvt->m_arrayVarType.getType();
        nvt->VerifyFlags(isInProcedure);
    ...
    if (m_currentToken.m_type == TokenType::AT ||
        ...
        //        Eat(m_currentToken.m_type);
        QSharedPointer<NodeVarType> nt =
            QSharedPointer<NodeVarType>(new NodeVarType(t, position));
        //        qDebug() << "IN TYPESPEC : " << t.m_value <<
        //        TokenType::getType(t.m_type);
    ...
    if (m_currentToken.m_type == TokenType::EQUALS) {
        ...
        Eat();
        //        data <<
        //        "$0"+QString::number(getIntVal(m_currentToken),16);//QString::number(m_currentToken.m_intVal);
        ...
        if (Syntax::s.m_currentSystem->m_system == AbstractSystem::NES ||
            ...
            Syntax::s.m_currentSystem->m_system == AbstractSystem::COLECO)
            ErrorHandler::e.Error(
                "You cannot initialize variables stored in work ram (WRAM) on the "
                "Gameboy/NES, as these needs to be initialized manually. ",
                m_currentToken.m_lineNumber - 1);
    }
    VerifyTypeSpec(t);
    ...
    QSharedPointer<NodeVarType> nvt =
        QSharedPointer<NodeVarType>(new NodeVarType(t, initVal));
    nvt->VerifyFlags(isInProcedure);
...
QSharedPointer<Node> Parser::BuiltinFunction() {
    if (Syntax::s.builtInFunctions.contains(m_currentToken.m_value.toLower())) {
        ...
        while (m_currentToken.m_type != TokenType::RPAREN) {
            if (m_currentToken.m_type == TokenType::STRING) {
                ...
                try {
                    ...
                    QString em = "Could not find symbol '<font color=\"#FF8000\">" +
                                 prev + "</font>'<br>";
                    QString similarSymbol =
                        m_symTab->findSimilarSymbol(prev, 65, 2, m_procedures.keys());
                    if (similarSymbol != "") {
                        em += "Did you mean '<font color=\"#A080FF\">" + similarSymbol +
                              "</font>'?<br>";
                    }
        ...
        if (noParams != paramList.count()) {
            ...
            s += "Requires " + QString::number(noParams) + " parameters but has " +
                 QString::number(paramList.count());
    ...
    for (auto p:paramList)
        ...
        */
        return QSharedPointer<NodeBuiltinMethod>(new NodeBuiltinMethod(
            procName, paramList, &Syntax::s.builtInFunctions[procName]));
        // p->SetParameters(paramList);
    }
    return nullptr;
}
...
QSharedPointer<Node> Parser::InlineAssembler() {
    ...
    if (m_settingsIni->getString("assembler_z80") == "Pasmo" &&
        Syntax::s.m_currentSystem->m_processor == AbstractSystem::Z80) {
...
void Parser::HandleImportChar() {
    ...
    if (imgA == nullptr) {
        ErrorHandler::e.Error("Importing char error : unknown filetype for '" +
                              inFile + "'");
    }
...
QStringList Parser::BuildTable(int cnt, TokenType::Type type) {
    ...
    if (cnt == 0)
        ErrorHandler::e.Error("BuildTable must have at least 1 element in array.",
                              m_currentToken.m_lineNumber);
    ...
    for (QString &key : m_symTab->m_userConstants)
        consts += key + "=" +
                  QString::number(m_symTab->m_constants[key]->m_value->m_fVal) +
                  ";";
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        //        })");
        if (fun.isError())
            ErrorHandler::e.Error("Error evaluation javascript expression : " +
                                      fun.toString() + " <br><br>",
                                  m_currentToken.m_lineNumber);
...
QStringList Parser::BuildSineTable(int cnt, TokenType::Type type) {
    ...
    for (int i = 0; i < cnt; i++) {
        QString str = "Math.sin(i/" + QString::number(cnt) + ".0*Math.PI*2)*" +
                      amp + " +" + amp;
        QJSValue fun = m_jsEngine.evaluate("(function(i) { " + consts + ";return " +
                                           str + "; })");
        if (fun.isError())
...
QStringList Parser::BuildTable2D(int cnt, TokenType::Type type) {
    ...
    if (x == 0 || y == 0)
        ErrorHandler::e.Error("Width / height of 2d table cannot be zero.",
                              m_currentToken.m_lineNumber);
    Eat(TokenType::COMMA);
    QString sentence = m_currentToken.m_value;
    Eat(TokenType::STRING);
...
void Parser::HandleCallMacro(QString name, bool ignore) {
    ...
    //    if (m_pass!=1)
    //       return;
    //        qDebug() << "Before " <<m_pass <<m_currentToken.m_value <<
    //        m_lexer->m_pos << m_lexer->m_text[m_lexer->m_pos];
...
void Parser::HandleBin2Inc() {
    ...
    if (!QFile::exists(inFile))
        ErrorHandler::e.Error("Bin2Inc error: could not open file " + inFile,
                              m_currentToken.m_lineNumber);

    QByteArray data = Util::loadBinaryFile(inFile);
    QString d = "var " + name + " : array[" + QString::number(data.size()) +
                "] of byte =( \n\t";
...
void Parser::HandlePathTool() {
    ...
    if (!QFile::exists(inFile))
        ErrorHandler::e.Error("Pathtool error: could not open file " + inFile,
                              m_currentToken.m_lineNumber);
...
void Parser::HandleCompress() {
    ...
    if (!QFile::exists(lz4))
        ErrorHandler::e.Error(
            "In order to compress files, please set up the 'lz4' path in the "
            "'Utilities' section in the TRSE settings panel.",
            m_currentToken.m_lineNumber);
    QString out, err;
    if (split != 1) {
...
void Parser::HandleExportParallaxData() {
    ...
    Compression::GenerateParallaxData(inFile, outFile, x0, y0, x1, y1, param1,
                                      param2, param3);
}
...
void Parser::GenerateUnrolledAsm1() {
    ...
    //    Compression::Generate(inFile, outFile, x0,y0,x1,y1,param1,param2,
    //    param3);
    Compression::GenerateUnrolledAsm1(name, outFile, inFile, src, dst, width,
                                      height, shift1, shift2, shift3);
}
...
void Parser::HandleExportBW() {
    ...
    if (!QFile::exists(inFile))
        ErrorHandler::e.Error("Could not find file :" + inFile,
                              m_currentToken.m_lineNumber);
...
void Parser::HandleBuildPaw() {
    ...
    QString inFile =
        m_currentDir + "/" +
                     m_currentToken.m_value; //    QString inFile = m_currentToken.m_value;form
    Eat(TokenType::STRING);
    FormPaw fp;
    fp.InitDocument(nullptr, m_settingsIni, m_projectIni);
    if (!QFile::exists(inFile))
        ErrorHandler::e.Error("Could not locate paw file for building: " + inFile,
                              ln);
    //    fp.Load(inFile);
...
void Parser::HandleExportPrg2Bin() {
    ...
    //    qDebug() << "Exporting prg "<<inFile<<outFile << Util::numToHex(from) <<
    //    Util::numToHex(to);
    ...
    //       Util::numToHex((uchar)in[0]) << Util::numToHex((uchar)in[1]);
    in = in.remove(0, 2);
    ...
    for (int i = from; i < to; i++) {
        ...
        if (in.length() < j || j < 0) {
            ...
            //          ErrorHandler::e.Error("ExportPrg2Bin error: .prg file does not
            //          contain specified binary range.",
            //          m_currentToken.m_lineNumber);
            if (j >= 0 && j < in.size())
                out.append(in[j]);
        }
...
void Parser::HandleVBMCompileChunk() {
    ...
    QString outFile =
        m_currentDir + "/" +
                      m_currentToken.m_value; // m_currentDir+"/"+ m_currentToken.m_value;
    Eat(TokenType::STRING);
    ...
    img->VBMCompileChunk(f, procName, pointerName, asmOperation, param1, param2,
                         param3, param4);
...
void Parser::HandleAKGCompiler() {
    ...
    if (!Tool::AKGCompiler(m_currentDir + filename, addr, m_symTab.get()))
        ErrorHandler::e.Error("Could not find music for inclusion : " + filename +
                              ".asm");
}
...
void Parser::HandleKrillsLoader() {
    ...
    QString track = "19";
    if (m_projectIni->contains("use_track_19") &&
        m_projectIni->getdouble("use_track_19") != 1.0)
        track = "18";

    //                      m_preprocessorDefines["_InstallKrill"] =
    //                      Util::numToHex(installerPos + 0x1390);
    ...
    //                    m_preprocessorDefines["_LoadrawKrill"] =
    //                    Util::numToHex(loaderPos); qDebug() <<
    //                    m_preprocessorDefines["_LoadrawKrill"];
    m_preprocessorDefines["_ResidentLoaderSource"] = Util::numToHex(loaderOrgPos);
    m_preprocessorDefines["_ResidentLoaderDestination"] =
        Util::numToHex(loaderPos);

    QString pos = QString::number(loaderPos, 16);
    if (pos == "200")
        pos = "0200";
    QString loaderFile = ":resources/bin/krill_" + track + "/loader_PAL_NTSC_" +
                         pos.toUpper() + "-c64.prg";
    QString installerFile =
        ":resources/bin/krill_" + track + "/install_PAL_NTSC_" +
                            QString::number(installerPos, 16).toUpper() + "-c64.prg";

    if (!QFile::exists(loaderFile))
        ErrorHandler::e.Error("When using krills loader, the loader location must "
                              "be either 0200, 1000,2000 etc");

    if (!QFile::exists(installerFile))
        ErrorHandler::e.Error("When using krills loader, the installer location "
                              "must be either 1000, 2000, 3000 etc");
    ...
    replaceLine += "_ResidentLoader_Binary: 	incbin (\"" + outFile + "\",$" +
                   QString::number(loaderOrgPos, 16) + ");";
    outFile = outFolderShort + "krill_installer.bin";
    replaceLine += "\n_Installer_Binary: 	incbin (\"" + outFile + "\",$" +
                   QString::number(installerPos, 16) + ");";

    for (QString s : m_diskFiles) {
        QString var = s;
        if (s == "")
            ErrorHandler::e.Error("One of your disk files is missing a name. Please "
                                  "correct the .paw file!");
        for (int i = 0; i < 256; i++) {
...
void Parser::HandleSpriteCompiler() {
    ...
    if (!QFile::exists(m_currentDir + "/" + filename))
        ErrorHandler::e.Error("Could not find file : " + filename,
                              m_currentToken.m_lineNumber);

    LImage *img = LImageIO::Load(m_currentDir + "/" + filename);
    m_parserAppendix << img->SpriteCompiler(name, m_currentDir, "", "", x, y, w,
                                            h, pparam);
...
void Parser::HandleSpritePacker() {
    ...
    if (imgChrOut == nullptr)
        ErrorHandler::e.Error(
            "Unknown image type : " + type +
                ". For now, only 'gameboy' and 'snes' is supported.",
            m_currentToken.m_lineNumber);
    ...
    if (imgSrc == nullptr) {
        ErrorHandler::e.Error(
            "Importing char error : unknown filetype for input binary '" + inFile +
            "'");
    }
    ...
    int noChars = 0;
    imgChrOut->SpritePacker(imgSrc, rawDataOut, spriteData, x, y, w, h, comp,
                            noChars);
    ...
    ErrorHandler::e.Warning("Added new sprite data from '" + inFile +
                                "' : sprite from " + QString::number(curPos) +
                                "  to " + QString::number(spriteData.size()) +
                                ". Sprite size: " + QString::number(noChars),
                            m_currentToken.m_lineNumber);
...
void Parser::HandleProjectSettingsPreprocessors() {
    ...
    if (cmd == "system") {
        if (m_projectIni->getString("system") != val) {
            m_abort = true;
            //       Syntax::s.Init(AbstractSystem::SystemFromString(val),m_settingsIni,
            //       m_projectIni);
...
void Parser::HandleUseTPU(QString fileName) {
    ...
    dirs << Util::GetSystemPrefix() + Data::data.unitPath + QDir::separator() +
                AbstractSystem::StringFromSystem(
                Syntax::s.m_currentSystem->m_system) +
                QDir::separator();
    // Special case for Z80
    QString cpu = AbstractSystem::StringFromProcessor(
        Syntax::s.m_currentSystem->m_processor);
    if (cpu == "Z180")
        cpu = "Z80";
    dirs << Util::GetSystemPrefix() + Data::data.unitPath + QDir::separator() +
                Data::data.cpuUnitPath + QDir::separator() + cpu +
                QDir::separator();
    dirs << Util::GetSystemPrefix() + Data::data.unitPath + QDir::separator() +
                "global" + QDir::separator();

    QString fname = Util::findFileInDirectories(fileName + ".tru", dirs);

    if (fname == "" || !QFile::exists(fname)) {
        ErrorHandler::e.Error(
            "Could not find TRU file for inclusion : " + fileName +
                " <br><br>( Tried dirs: " + Util::toString(dirs) + ")",
            m_currentToken.m_lineNumber);
    }
...
*/
    ...
    try {
        ...
        e.message = "<font color=\"#FFB030\">Error during compiling the Turbo "
                    "Rascal Unit file : '</font><font color=\"yellow\">" +
                    fileName + "</font>'<font color=\"#FFB030\"> on line " +
                    QString::number(e.linenr) + "</font><br><font color=\"red\">" +
                    e.message + "</font>";
...
QSharedPointer<Node> Parser::Expr() {
    ...
    if (!m_isInConditional)
        while (m_currentToken.m_type == TokenType::Type::GREATER ||
               m_currentToken.m_type == TokenType::Type::GREATEREQUAL ||
               m_currentToken.m_type == TokenType::Type::LESS ||
               m_currentToken.m_type == TokenType::Type::LESSEQUAL ||
               m_currentToken.m_type == TokenType::Type::EQUALS ||
               m_currentToken.m_type == TokenType::Type::NOTEQUALS ||
               m_currentToken.m_type == TokenType::Type::AND ||
               m_currentToken.m_type == TokenType::Type::OR

               ) {
            if (m_currentToken.m_type == TokenType::Type::AND ||
                m_currentToken.m_type == TokenType::Type::OR) {
                ErrorHandler::e.Error(
    ...
    while (m_currentToken.m_type == TokenType::Type::PLUS ||
           m_currentToken.m_type == TokenType::Type::MINUS ||
           m_currentToken.m_type == TokenType::Type::BITAND ||
           m_currentToken.m_type == TokenType::Type::BITOR) {
        Token t = m_currentToken;
    ...
    if (node->isPureNumeric() &&
        ...
        //        qDebug() << "Collapsing node to " << Util::numToHex(val) <<
        //        t.getType() << t.m_lineNumber << node->m_op.getType();
  ...
  if (node->m_op.m_type==TokenType::DIV) {
      if (node->m_right->isPureNumeric()) {
          ...
          if (div2s.contains(val)) {
              ...
              qSharedPointerDynamicCast<NodeNumber>(node->m_right)->m_val =
div2s.indexOf(val)+1;
          }
      }
  }
