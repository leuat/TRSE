...
slotss are reference counted to help mitigate the ABA problem in the lock-free queue we use for tracking jobs.
...
MA_API ma_result ma_slot_allocator_alloc(ma_slot_allocator* pAllocator, ma_uint64* pSlot)
{
    ma_uint32 capacity;
    ma_uint32 iAttempt;
    const ma_uint32 maxAttempts = 2;    /* The number of iterations to perform until returning MA_OUT_OF_MEMORY if no slotss can be found. */
        ...
        for (iGroup = 0; iGroup < ma_countof(pAllocator->groups); iGroup += 1) {
            ...
            for (;;) {
                ...
                if (c89atomic_compare_and_swap_32(&pAllocator->groups[iGroup].bitfield, oldBitfield, newBitfield) == oldBitfield) {
                    ...
                    pAllocator->slotss[slotIndex] += 1;

                    /* Construct the output value. */
                    *pSlot = ((ma_uint64)pAllocator->slotss[slotIndex] << 32 | slotIndex);
...
MA_API ma_result ma_job_queue_post(ma_job_queue* pQueue, const ma_job* pJob)
{
    ...
    if (result != MA_SUCCESS) {
        return result;  /* Probably ran out of slotss. If so, MA_OUT_OF_MEMORY will be returned. */
    }
