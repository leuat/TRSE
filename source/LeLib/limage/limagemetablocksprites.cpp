...
//        if (rand()%100>98)
  //        qDebug() << QString::number(color);
    ...
    if (m_writeType==Color)
        ((LMetaChunkItem*)m_items[m_current])->setPixelAttrib(p.x(),p.y(),color&3,m_img->m_bitMask);
...
unsigned int LImageMetaBlockSprites::getPixel(int x, int y)
{
    ...
    uchar pal = ((LMetaChunkItem*)m_items[m_current])->getPixelAttrib(p.x(),p.y(),m_img->m_bitMask);
    pal = (pal&3)+4;
